...
exports.tokenize = function(code, o){
  ...
  while (code = code.slice(i)) {
    ...
    case '\\':
      ...
      break;
    case '\'':
      i = this.doHeredoc(code, '\'') || this.singleStringToken(code);
      break;
    case '"':
      i = this.doHeredoc(code, '"') || this.doubleStringToken(code);
      break;
    case '<':
...
exports.doLine = function(it){
  var input, indent, last, tag, that, continued, _ref;
  _ref = MULTIDENT.exec(it), input = _ref[0], indent = _ref[1];
  this.countLines(input);
  this.seenFrom = this.seenTo = false;
  last = this.last;
  tag = (last.eol = true, last.spaced = true, last)[0];
  if ((tag === '+-' || tag === 'DOT' || tag === 'LOGIC' || tag === 'MATH' || tag === 'COMPARE' || tag === 'RELATION' || tag === 'SHIFT' || tag === 'BITWISE') || tag === 'ASSIGN' && ((_ref = '' + last[1]) !== '=' && _ref !== ':=')) {
    return input.length;
  }
  if (that = indent && (this.emender || (this.emender = RegExp('[^' + indent.charAt(0) + ']'))).exec(indent)) {
    this.carp("contaminated indent " + escape(that));
  }
  continued = LINE_CONTINUER.test(it);
  if (this.indent < (indent = indent.length)) {
    if (continued) {
      return input.length;
    }
    this.indents.push(this.token('INDENT', indent - this.indent + this.debt));
    this.debt = 0;
  } else {
    this.dedent(this.indent - indent, continued);
  }
  this.indent = indent;
  return input.length;
};
...
exports.dedent = function(tab, noNewline){
  var idt, _ref;
  while (tab > 0) {
    if (!(idt = (_ref = this.indents)[_ref.length - 1])) {
      tab = 0;
    } else if (idt <= this.debt) {
      tab -= idt;
      this.debt -= idt;
    } else {
      tab -= this.token('DEDENT', this.indents.pop() - this.debt);
      this.debt = 0;
    }
  }
  this.debt -= tab;
