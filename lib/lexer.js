(function(){
  var CALLABLE, COCO_ALIASES, COCO_KEYWORDS, COMMENTS, HERECOMMENT, HEREDOC_INDENT, HEREDOUBLE, HEREGEX, HEREGEX_OMIT, HERESINGLE, IDENTIFIER, INDEXABLE, JSTOKEN, JS_FORBIDDEN, JS_KEYWORDS, LINE_CONTINUER, Lexer, MULTIDENT, MULTILINER, NUMBER, REGEX, RESERVED, Rewriter, SIMPLESTR, SYMBOL, TRAILING_SPACES, WHITESPACE, WORDS, __indexOf = Array.prototype.indexOf || function(x){
  ...
  exports.Lexer = Lexer = (function(){
    ...
    _ref.tokenize = function(code, o){
      ...
      while (this.chunk = code.slice(i)) {
        ...
        case ' ':
          ...
          break;
        case "'":
          i += this.heredocToken(HERESINGLE) || this.singleStringToken();
          break;
        case '"':
          i += this.heredocToken(HEREDOUBLE) || this.doubleStringToken();
          break;
        case '/':
          i += this.heregexToken() || this.regexToken() || this.literalToken();
          break;
        case '<':
          i += this.wordsToken() || this.literalToken();
          break;
        case '#':
          i += this.commentToken();
          break;
    ...
    _ref.singleStringToken = function(){
      ...
      if (!(string = SIMPLESTR.exec(this.chunk))) {
        this.carp('unterminated single quoted string');
      }
    ...
    _ref.heredocToken = function(regex){
      ...
      if (quote === '"' && 0 <= doc.indexOf('#{')) {
        ...
        this.token('STRNUM', this.makeString(doc, quote, '\\n'));
      }
      return this.countLines(heredoc).length;
    };
    _ref.commentToken = function(){
      var match;
      match = HERECOMMENT.exec(this.chunk) || [this.chunk, this.chunk.slice(3)];
    ...
    _ref.wordsToken = function(){
      var call, word, words, _i, _len, _ref;
    ...
    _ref.lineToken = function(){
      ...
      noNewlines = LINE_CONTINUER.test(this.chunk) || ((_ref = this.last[0]) === "ACCESS" || _ref === "INDEX_START" || _ref === "ASSIGN" || _ref === "COMPOUND_ASSIGN" || _ref === "IMPORT" || _ref === "LOGIC" || _ref === "PLUS_MINUS" || _ref === "MATH" || _ref === "COMPARE" || _ref === "RELATION" || _ref === "SHIFT");
