(function(){
  ...
  exports.Lexer = Lexer = (function(){
    ...
    _ref.heregexToken = function(){
      var body, bs, flags, heregex, match, token, tokens, val, _i, _len, _ref;
      ...
      tokens = [];
      for (_i = 0, _len = (_ref = this.interpolateString(body)).length; _i < _len; ++_i) {
        token = _ref[_i];
        if (token[0] === 'TOKENS') {
          tokens.push.apply(tokens, token[1]);
        } else {
          if (!(val = token[1].replace(HEREGEX_OMIT, ''))) {
            continue;
          }
          val = val.replace(bs || (bs = /\\/g), '\\\\');
          tokens.push(['STRNUM', this.makeString(val, '"', '\\n')]);
        }
    ...
    _ref.interpolateString = function(str, newline){
      var chr, code, i, nested, pi, tag, tokens, value, _len, _ref;
      tokens = [];
      pi = 0;
      i = -1;
      ...
      if (pi < str.length) {
        tokens.push(['S', str.slice(pi)]);
      }
      if (((_ref = tokens[0]) != null ? _ref[0] : void 8) !== 'S') {
        tokens.unshift(['S', '']);
      }
      if (newline == null) {
        return tokens;
      ...
      for (i = 0, _len = tokens.length; i < _len; ++i) {
        ...
        if (tag === 'TOKENS') {
          ...
          this.token('STRNUM', this.makeString(value, '"', newline));
        }
      }
      this.token(")", ")");
      return tokens;
