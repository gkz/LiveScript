var rewrite, able, string, TABS, INDENTS, LINES, enslashes, JS_KEYWORDS, COCO_ALIASES, RESERVED, ID, SYMBOL, SPACE, MULTIDENT, SIMPLESTR, JSTOKEN, BSTOKEN, NUMBER, NUMBER_OMIT, REGEX, HEREGEX_OMIT, LASTDENT, LINE_CONTINUER, _ref, __clone = function(it){
...
exports.doNumber = function(code, lastIndex){
  var match, input, num, last, radix, rnum, sign, dotpos, _ref;
...
exports.doHeregex = function(code, index){
  ...
  flag = this.validate(/^[imgy]{0,4}/.exec(rest)[0]);
  if (parts[1]) {
    ...
    this.regex(enslashes(parts[0][1].replace(HEREGEX_OMIT, '')), flag);
  }
...
exports.interpolate = function(str, idx, end){
  var parts, end0, pos, i, ch, s, clone, nested, delta, _ref;
  parts = [];
  end0 = end.charAt(0);
  pos = 0;
  i = -1;
...
exports.pair = function(it){
  var wanted, _ref;
  if (it !== (wanted = (_ref = this.closes)[_ref.length - 1])) {
    if (!('DEDENT' === wanted && it === (_ref = this.closes)[_ref.length - 2])) {
...
LINES = /\n/g;
enslashes = (function(re, fn){
  return function(it){
    return it.replace(re, fn);
  };
}.call(this, /(\\.)|\//g, function(_, quoted){
  return quoted || '\\/';
}));
function lchomp(it){
...
JSTOKEN = /`[^\\`]*(?:\\[\s\S][^\\`]*)*`|/g;
BSTOKEN = /\\(?:(\S[^\s,;)}\]]*)|\s*)/g;
NUMBER = /0x[\da-f][\da-f_]*|([2-9]|[12]\d|3[0-6])r([\da-z][\da-z_]*)|((?:\d[\d_]*(?:\.\d[\d_]*)?|\.\d[\d_]*)(?:e[+-]?\d[\d_]*)?)[a-z_]*|/ig;
NUMBER_OMIT = /_+/g;
REGEX = /\/([^[\/\n\\]*(?:(?:\\.|\[[^\]\n\\]*(?:\\.[^\]\n\\]*)*\])[^[\/\n\\]*)*)\/([imgy]{0,4})|/g;
HEREGEX_OMIT = /\s+(?:#.*)?/g;
LASTDENT = /\n[^\n\S]*$/;
