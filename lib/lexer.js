...
exports.doLiteral = function(code, index){
  ...
  case ')':
    ...
    break;
  case ':':
    if ((__ref = this.last[0]) != 'ID' && __ref != 'STRNUM' && __ref != ')' && __ref != ')CALL') {
      tag = 'LABEL';
      val = '';
    }
    break;
...
function addImplicitFunctions(tokens){
  var i, token, prev, j, t, __ref;
  i = 0;
  while (token = tokens[++i]) {
    if ((__ref = token[0]) != 'ASSIGN' && __ref != ':') {
      continue;
    }
    prev = tokens[i - 1];
    if (prev[0] !== ')CALL') {
    ...
    j = i;
    t = tokens[j];
    while (!((t = tokens[--j])[0] === 'CALL(' || j === 0)) {}
    if (t[0] !== 'CALL(' || tokens[j - 1][0] !== 'ID') {
      continue;
    }
