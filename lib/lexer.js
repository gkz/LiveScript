var rewrite, able, string, untabify, unlines, enlines, JS_KEYWORDS, COCO_KEYWORDS, COCO_ALIASES, RESERVED, FORBIDDEN, IDENTIFIER, NUMBER, SYMBOL, SPACE, MULTIDENT, SIMPLESTR, JSTOKEN, REGEX, HEREGEX, HEREGEX_OMIT, LINE_CONTINUER, _ref, __clone = function(it){
...
exports.identifierToken = function(it){
  ...
  } else if (!prev[1] && ((_ref = prev[0]) === 'CATCH' || _ref === 'FUNCTION')) {
...
exports.commentToken = function(it){
  var end, text;
  text = it.slice(0, ~(end = it.indexOf('*/', 2)) ? end + 2 : 9e9);
...
exports.jsToken = function(it){
  ...
  this.token('LITERAL', new String(untabify((js = js[0]).slice(1, -1), this.indent)));
  this.last[1].js = true;
  return this.countLines(js).length;
};
exports.regexToken = function(it){
  var prev, regex, _ref;
  if (((_ref = (prev = this.last)[0]) === 'LITERAL' || _ref === 'CREMENT') || this.able(true) || !(regex = REGEX.exec(it))) {
    return 0;
  }
...
exports.interpolate = function(str, nlines){
  var line, ts, pi, i, that, s, code, nested, len, tokens, t, _ref, _len;
  line = this.line;
  ts = [];
  pi = 0;
  i = -1;
...
exports.strnum = function(it){
  if (this.last[0] !== 'STRNUM' && this.able()) {
    this.token('DOT', '.');
  }
