...
exports.doLiteral = function(code, index){
  var sym, val, tag, arrow, i, t, curried, that, up, __ref;
  ...
  case '=':
  case ':':
    if (this.last[0] === 'UNARY' && this.last[1] === '!') {
      this.tokens.pop();
      this.token('CALL(', '(');
      this.token(')CALL', ')');
    }
    if (this.last[0] === ')CALL') {
      ...
      if (this.tokens[i - 2][1] === '&' && this.tokens[i - 3][1] === 'this') {
        this.tokens.splice(i - 2, 0, ['DOT', '.', this.line]);
        i++;
      } else if (((__ref = this.tokens[i - 3]) != null ? __ref[1] : void 8) === '&' && this.tokens[i - 4][1] === 'this') {
        this.tokens.splice(i - 3, 0, ['DOT', '.', this.line]);
        i++;
      }
      curried = this.tokens[i - 1][0] === 'ID' && this.tokens[i - 2][1] === '&'
        ? (this.tokens.splice(i - 2, 1), true)
        : this.tokens[i][1] === '!' && this.tokens[i - 3][1] === '&' ? (this.tokens.splice(i - 3, 1), true) : void 8;
      if (curried) {
        arrow = arrow.charAt(0) + arrow;
      }
      this.token('->', arrow);
      return 2;
    }
  ...
  if (tag == ',' || tag == 'PIPE' || tag == 'DOT' || tag == 'LOGIC' || tag == 'COMPARE' || tag == 'MATH' || tag == 'POWER' || tag == 'IMPORT' || tag == 'SHIFT' || tag == 'BITWISE') {
    this.unline();
  }
  this.token(tag, val);
  return sym.length;
...
function addImplicitSwitches(tokens){
  var i, token, prev, __ref;
  i = 0;
  while (token = tokens[++i]) {
    prev = tokens[i - 1];
    if (!(token[0] === 'CASE' && ((__ref = prev[0]) == '->' || __ref == 'ASSIGN'))) {
      continue;
    }
