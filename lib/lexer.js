...
exports.doLiteral = function(code, index){
  var sym, nextSym, val, tag, future, additional, switchOps, num, that, up, __ref;
  ...
  case '&&':
  case '||':
    if (((__ref = future = nextSym(2)) == '&' || __ref == '|') && future !== '=') {
      sym = val = val + val[0];
      tag = 'BITWISE';
    } else {
      tag = 'LOGIC';
