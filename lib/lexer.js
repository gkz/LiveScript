...
exports.identifierToken = function(it){
  var match, id, at, tag, input, colon, prev, forcedIdentifier, attr, _ref, _ref2;
  ...
  } else if (this.last[0] === 'CATCH' && !this.last[1]) {
    return (this.last[1] = id).length;
  }
  this.token(tag, id);
  if (attr) {
    this.last[attr] = true;
...
exports.numberToken = function(it){
  ...
  switch (num.charAt(0)) {
  case '.':
    if (this.callable() || ((_ref = this.last[0]) === '}' || _ref === 'LITERAL')) {
      this.token('DOT', '.');
...
exports.lineToken = function(it){
  ...
  noNewline = LINE_CONTINUER.test(it) || ((_ref = this.last[0]) === '+-' || _ref === 'DOT' || _ref === 'ASSIGN' || _ref === 'LOGIC' || _ref === 'MATH' || _ref === 'COMPARE' || _ref === 'RELATION' || _ref === 'SHIFT' || _ref === 'IMPORT');
...
exports.literalToken = function(it){
  ...
  default:
    ...
    if (val === '(' && this.callable()) {
      if (this.last[0] === '?') {
        _ref2 = ['CALL(', '?('], (_ref = this.last)[0] = _ref2[0], _ref[1] = _ref2[1];
        return val.length;
      }
      tag = 'CALL(';
    }
