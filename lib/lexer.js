var rewrite, able, string, detab, unlines, enlines, JS_KEYWORDS, COCO_ALIASES, RESERVED, ID, NUMBER, SYMBOL, SPACE, MULTIDENT, SIMPLESTR, JSTOKEN, WORD, REGEX, HEREGEX, HEREGEX_OMIT, LINE_CONTINUER, _ref, __clone = function(it){
...
exports.doLine = function(it){
  var input, tabs, last, length, dent, that, tag, _ref;
  ...
  if (dent < this.dent) {
    ...
    if (this.dent < dent) {
      this.dents.push(this.token('INDENT', dent - this.dent));
    } else {
      this.newline();
    }
  }
...
exports.doLiteral = function(it){
  var val, tag, that, tokens, i, _ref;
  ...
  case ':':
    ...
    break;
  case '*':
    if (that = ((_ref = this.last[0]) === 'INDENT' || _ref === 'TERMINATOR') && /^.[^\n\S]*(?=\S)/.exec(it)) {
      this.dent += val = that[0].length;
      this.tokens.push(['{', '{', this.line], ['}', '}', this.line], ['ASSIGN', '=', this.line]);
      this.dents.push(this.token('INDENT', val));
      return val;
    }
...
exports.dedent = function(count){
  var dent;
  while (count > 0 && (dent = this.dents.pop())) {
    if (count < dent) {
      this.carp("unmatched dedent (" + count + " for " + dent + ")");
    }
    count -= this.token('DEDENT', dent);
  }
};
