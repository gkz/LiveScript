var rewrite, able, string, INDENTS, LINES, JS_KEYWORDS, COCO_ALIASES, RESERVED, ID, SYMBOL, SPACE, MULTIDENT, SIMPLESTR, JSTOKEN, BSTOKEN, NUMBER, NUMBER_OMIT, REGEX, HEREGEX, HEREGEX_OMIT, LINE_CONTINUER, _ref, __clone = function(it){
...
exports.doRegex = function(it){
  var divisable, match;
  if ((divisable = able(this.tokens)) && this.last.spaced) {
    if (/^\/[ =]/.test(it) || !(match = REGEX.exec(it))) {
      return 0;
    }
  } else {
    if (divisable) {
