...
exports.doLiteral = function(code, index){
  var sym, val, tag, future, additional, switchOps, num, that, up, __ref;
  ...
  case '&&':
  case '||':
    if (((__ref = future = code.charAt(index + 2)) == '&' || __ref == '|') && future !== '=') {
      sym = val = val + val[0];
      tag = 'BITWISE';
    } else {
      tag = 'LOGIC';
  ...
  case '<<<':
    if (code.charAt(index + 3) === '<') {
      additional = code.charAt(index + 4) === '<' ? '<' : '';
