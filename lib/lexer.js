(function(){
  var CALLABLE, COFFEE_ALIASES, COFFEE_KEYWORDS, COMMENT, HEREDOC, HEREDOC_INDENT, HEREGEX, HEREGEX_OMIT, IDENTIFIER, INDEXABLE, JSTOKEN, JS_FORBIDDEN, JS_KEYWORDS, LEADING_SPACES, LINE_CONTINUER, Lexer, MULTILINER, MULTI_DENT, NUMBER, REGEX, RESERVED, Rewriter, SIMPLESTR, SYMBOL, TRAILING_SPACES, WHITESPACE, WORDS, op, __indexOf = Array.prototype.indexOf || function(item){
  ...
  exports.Lexer = (function(){
    ...
    Lexer.prototype.identifierToken = function(){
      ...
      forcedIdentifier = at || colon || ((prev = (_ref = this.tokens)[_ref.length - 1])[1].colon2 ? this.token('ACCESS', '.') : prev[0] === 'ACCESS');
    ...
    Lexer.prototype.jsToken = function(){
      var js;
      if (!(this.chunk.charAt(0) === '`' && (js = JSTOKEN.exec(this.chunk)))) {
        return 0;
      }
    ...
    Lexer.prototype.regexToken = function(){
      ...
      if (regex = HEREGEX.exec(this.chunk)) {
        return this.heregexToken(regex);
      }
      if (((_ref = this.tag()) === "STRNUM" || _ref === "LITERAL" || _ref === "++" || _ref === "--") || !(regex = REGEX.exec(this.chunk))) {
        return 0;
      }
    ...
    Lexer.prototype.heregexToken = function(match){
      var body, flags, heregex, tag, tokens, value, _i, _len, _ref;
    ...
    Lexer.prototype.wordsToken = function(){
      ...
      this.token('[', '[');
      _ref = (words = words[0]).slice(2, -2).match(/\S+/g) || [''];
    ...
    Lexer.prototype.lineToken = function(){
      var diff, indent, noNewlines, size;
      if (!(indent = MULTI_DENT.exec(this.chunk))) {
        return 0;
      }
    ...
    Lexer.prototype.whitespaceToken = function(){
      var match, _ref;
      if (!((match = WHITESPACE.exec(this.chunk)) || this.chunk.charAt(0) === '\n')) {
        return 0;
      }
      (_ref = this.tokens)[_ref.length - 1][match ? 'spaced' : 'eol'] = true;
      if (match) {
    ...
    Lexer.prototype.sanitizeHeredoc = function(doc, options){
      var attempt, herecomment, indent, _ref;
    ...
    Lexer.prototype.countLines = function(str){
      var num, pos;
      num = pos = 0;
