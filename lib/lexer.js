(function(){
  var JS_KEYWORDS, COCO_KEYWORDS, COCO_ALIASES, RESERVED, JS_FORBIDDEN, IDENTIFIER, NUMBER, SYMBOL, SPACE, MULTIDENT, SIMPLESTR, JSTOKEN, REGEX, HEREGEX, HEREGEX_OMIT, MULTILINER, LINE_CONTINUER, CALLABLE, INDEXABLE, _ref, __indexOf = Array.prototype.indexOf || function(x){
    for (var i = this.length; i-- && this[i] !== x;); return i;
  };
  _ref = (exports.Lexer = (function(){
    function Lexer(){}
    return Lexer;
  }())).prototype;
  ...
  _ref.heredocToken = function(q){
    ...
    if (q === '"' && ~doc.indexOf('#{')) {
      ...
      this.token('STRNUM', this.string(doc, q, '\\n'));
      this.countLines(doc);
    }
    if (lnl) {
      ++this.line;
  ...
  _ref.commentToken = function(){
    ...
    this.token('COMMENT', this.dedent(text, this.indent));
  ...
  _ref.heregexToken = function(){
    ...
    for (i = 0, _len = (_ref = this.interpolateString(body)).length; i < _len; ++i) {
      ...
      if (token[0] === 'TOKENS') {
        ...
        tokens.push(['STRNUM', this.string(val, "'", '\\n'), token[2]]);
      }
      tokens.push(['PLUS_MINUS', '+', tokens[tokens.length - 1][2]]);
    }
    tokens.pop();
    if (flags) {
      this.token(',', ',');
  ...
  _ref.wordsToken = function(){
    var end, call, line, re, words, word, _ref, _i, _len, _j, _len2;
    ...
    for (_i = 0, _len = (_ref = this.chunk.slice(2, end).split('\n')).length; _i < _len; ++_i) {
      line = _ref[_i];
      if (words = line.match(re || (re = /\S+/g))) {
        for (_j = 0, _len2 = words.length; _j < _len2; ++_j) {
          word = words[_j];
          this.tokens.push(['STRNUM', this.string(word, "'"), this.line], [',', ',', this.line]);
        }
      }
      ++this.line;
    }
  ...
  _ref.spaceToken = function(){
    var match;
    if (!(match = SPACE.exec(this.chunk))) {
      return 0;
    }
  ...
  _ref.literalToken = function(){
    ...
    case '=>':
      ...
      break;
    case '*':
      tag = (_ref = this.last[0]) === 'INDEX_START' || _ref === '(' || _ref === 'ACCESS' ? 'STRNUM' : 'MATH';
      break;
    case '=':
    case ':=':
    case '+=':
  ...
  _ref.balancedString = function(str, delimited){
  ...
  _ref.interpolateString = function(str, newline){
    var line, ts, pi, i, chr, s, code, nested, tokens, t, _ref, _len;
    line = this.line;
    ts = [];
    pi = 0;
    i = -1;
    while (chr = str.charAt(++i)) {
      ...
      nested = new this.constructor().tokenize(code, {
        line: this.line,
        rewrite: false
      });
      nested.pop();
    ...
    for (i = 0, _len = ts.length; i < _len; ++i) {
      ...
      if (t[0] === 'TOKENS') {
        ...
        tokens.push(['STRNUM', this.string(t[1], '"', newline), t[2]]);
      }
    }
    this.token(')', ')');
    return ts;
  ...
  _ref.string = function(body, quote, newline){
    if (!body) {
      return quote + quote;
    }
