...
exports.doID = function(code, lastIndex){
  ...
  if (colon || last[0] === 'DOT' || this.adi()) {
    ...
    case 'for':
      ...
      // fallthrough
    case 'then':
      this.seenFrom = this.wantBy = false;
      break;
    case 'catch':
    case 'function':
      id = '';
...
exports.doLine = function(code, index){
  ...
  if (0 > (delta = tabs.length - this.dent)) {
    ...
    if ((tag = last[0]) === 'ASSIGN' && ((_ref = '' + last[1]) !== '=' && _ref !== ':=' && _ref !== '+=') || (tag === '+-' || tag === '=>' || tag === 'DOT' || tag === 'LOGIC' || tag === 'MATH' || tag === 'COMPARE' || tag === 'RELATION' || tag === 'SHIFT' || tag === 'BITWISE' || tag === 'IN' || tag === 'OF' || tag === 'TO' || tag === 'BY' || tag === 'FROM')) {
      return length;
    }
    if (delta) {
      this.indent(delta);
...
exports.doLiteral = function(code, index){
  ...
  case '**=':
    ...
    } else if ((val === '+=' || val === '-=' || val === '^=') && !able(this.tokens) && ((_ref = this.last[0]) !== 'UNARY' && _ref !== '+-' && _ref !== '^')) {
...
function able(tokens, i, call){
  ...
  return (tag === 'ID' || tag === ']' || tag === '?' || tag === 'SUPER') || (call
...
CHAIN = ['(', '{', '[', 'ID', 'STRNUM', 'LITERAL', 'LET', 'WITH', 'WORDS', 'SUPER'];
