(function(){
  var Lexer, string, dedent, JS_KEYWORDS, COCO_KEYWORDS, COCO_ALIASES, RESERVED, JS_FORBIDDEN, IDENTIFIER, NUMBER, SYMBOL, SPACE, MULTIDENT, SIMPLESTR, JSTOKEN, REGEX, HEREGEX, HEREGEX_OMIT, MULTILINER, LINE_CONTINUER, CALLABLE, INDEXABLE, __clone = function(it){
    function fn(){ if (this.__proto__ !== it) this.__proto__ = it }
    return fn.prototype = it, new fn;
  }, __indexOf = Array.prototype.indexOf || function(x){
    for (var i = this.length; i-- && this[i] !== x;); return i;
  };
  Lexer = (exports.lex = function(code, options){
    return __clone(Lexer).tokenize(code, options || {});
  ...
  }, exports.outdent = function(moveOut, noNewline){
    var idt, _ref;
    while (moveOut > 0) {
