var string, TABS, unlines, enlines, enslash, reslash, character, KEYWORDS_SHARED, KEYWORDS_UNUSED, KEYWORDS, ID, SYMBOL, SPACE, MULTIDENT, SIMPLESTR, JSTOKEN, BSTOKEN, NUMBER, NUMBER_OMIT, REGEX, HEREGEX_OMIT, LASTDENT, OPENERS, CLOSERS, INVERSES, CHAIN, ARG, __slice = [].slice;
...
exports.doID = function(code, index){
  var match, input, id, last, tag, _ref;
  ...
  if (match[2] || last[0] === 'DOT' || this.adi()) {
    this.token('ID', __of(id, KEYWORDS) ? (_ref = Object(id), _ref.reserved = true, _ref) : id);
    if (match[2]) {
      this.token(':', ':');
    }
    return input.length;
...
exports.doJS = function(code, lastIndex){
  ...
  this.token('LITERAL', (_ref = Object(detab(js.slice(1, -1), this.dent)), _ref.js = true, _ref));
...
function addImplicitParentheses(tokens){
  var i, brackets, token, endi, tpair, tag, prev, seenSwitch, skipBlock, _ref;
  i = 0;
  brackets = [];
  while (token = tokens[++i]) {
    ...
    if (tag === 'CREMENT') {
      if (token.spaced || !__of((_ref = tokens[i + 1]) != null ? _ref[0] : void 8, CHAIN)) {
        continue;
      }
    }
