...
exports.doLine = function(it){
  var input, indent, last, tag, noNewline, size, that, _ref;
  _ref = MULTIDENT.exec(it), input = _ref[0], indent = _ref[1];
  this.countLines(input);
  this.seenFrom = this.seenTo = false;
  last = this.last;
  tag = last[0];
  noNewline = (tag === '+-' || tag === 'DOT' || tag === 'LOGIC' || tag === 'MATH' || tag === 'COMPARE' || tag === 'RELATION' || tag === 'IMPORT' || tag === 'SHIFT' || tag === 'BITWISE') || tag === 'ASSIGN' && ((_ref = '' + last[1]) !== '=' && _ref !== ':=') || LINE_CONTINUER.test(it);
