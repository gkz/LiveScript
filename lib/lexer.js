...
exports.regexToken = function(it){
  var prev, regex, _ref;
  if (((_ref = (prev = this.last)[0]) === 'STRNUM' || _ref === 'LITERAL' || _ref === 'CREMENT') || !prev.spaced && (_ref = prev[0], __indexOf.call(CALLABLE, _ref) >= 0) || !(regex = REGEX.exec(it))) {
    return 0;
  }
...
exports.lineToken = function(it){
  ...
  noNewline = LINE_CONTINUER.test(it) || ((_ref = this.last[0]) === '+-' || _ref === 'DOT' || _ref === 'INDEX[' || _ref === 'ASSIGN' || _ref === 'LOGIC' || _ref === 'MATH' || _ref === 'COMPARE' || _ref === 'RELATION' || _ref === 'SHIFT' || _ref === 'IMPORT');
...
exports.literalToken = function(it){
  ...
  case ';':
    ...
    break;
  case '*':
    tag = (_ref = this.last[0]) === 'INDEX[' || _ref === '(' || _ref === 'DOT' || _ref === ',' || _ref === ';' ? 'STRNUM' : 'MATH';
    break;
  case '::':
    this.token('DOT', '.');
  ...
  default:
    ...
    if (!(prev = this.last).spaced) {
      if (val === '(' && (_ref = prev[0], __indexOf.call(CALLABLE, _ref) >= 0)) {
        if (prev[0] === '?') {
          prev[0] = 'CALL(';
          prev[1] += '(';
          return val.length;
        ...
        tag = 'CALL(';
      } else if (val === '[' && (_ref = prev[0], __indexOf.call(INDEXABLE, _ref) >= 0)) {
        tag = 'INDEX[';
      }
    }
  }
