(function(){
  ...
  PR.registerLangHandler(PR.createSimpleLexer([['str', /^\\\S[^\s,;)}\]]*/, 0, '\\'], ['str', /^'(?:''[^]*?''|[^\\']*(?:\\.[^\\']*)*)'/, 0, '\''], ['lang-qq', /^("""[^]*?""")/, 0, '"'], ['lang-qq', /^("[^\\"]*(?:\\.[^\\"]*)*")/, 0, '"'], ['lang-qr', /^(\/{3}[^]+?\/{3}[imgy]{0,4})(?!\w)/, 0, '/'], ['com', /^\#(?:\##[^#][^]*?###|.*)/, 0, '#'], ['pln', /^[\.@][$A-Za-z_\x7f-\uffff][$\w\x7f-\uffff]*/, 0, '.@'], ['typ', /^(?:0x[\da-f]+|[1-9]\d?r[\da-z]+|(?:\d+(?:\.\d+)?)(?:e[+-]?\d+)?[a-z]*)/i, 0, '0123456789'], ['lang-js', /^`([^\\`]*(?:\\.[^\\`]*)*)`/, 0, '`']], [['kwd', /^(?:t(?:ry|h(?:row|en)|ypeof)|f(?:or(?:own)?|inally|unction)|n(?:ew|ot)|c(?:ontinue|a(?:se|tch)|lass)|i(?:f|n(?:stanceof)?|mport(?:all)?|s(?:nt)?)|e(?:lse|xtends)|d(?:e(?:fault|lete|bugger)|o)|un(?:less|til)|o[fr]|return|break|while|switch|and)\b(?![^\n\S]*:(?!:))/], ['typ', /^(?:true|false|null|void)\b(?![^\n\S]*:(?!:))/], ['pun', /^(?:(?:th(?:is|at)|it|super|arguments|eval)\b(?![^\n\S]*:(?!:))|@[@\d]?)/], ['tag', /^(?:Array|Boolean|Date|Error|Function|JSON|Math|Number|Object|RegExp|S(?:tring|yntaxError)|TypeError)\b(?![^\n\S]*:(?!:))/], ['str', /^<(?!<)\[[^]*?]>/], ['lang-r', /^[^\/](\/(?![\s\/])[^[\/\n\\]*(?:(?:\\[^]|\[[^\]\n\\]*(?:\\[^][^\]\n\\]*)*])[^[\/\n\\]*)*\/[imgy]{0,4})(?!\w)/]]), ['co']);
