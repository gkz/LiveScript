(function(){
  ...
  PR.registerLangHandler(PR.createSimpleLexer([['str', /^\\\S[^\s,;)}\]]*/, 0, '\\'], ['str', /^'(?:''[\s\S]*?''|[^\\']*(?:\\.[^\\']*)*)'/, 0, '\''], ['str', /^"(?:""[\s\S]*?""|[^\\"]*(?:\\.[^\\"]*)*)"/, 0, '"'], ['lit', /^`[^\\`]*(?:\\.[^\\`]*)*`/, 0, '`'], ['lit', /^<\[[\s\S]*?]>/, 0, '<'], ['lit', /^(?:\/{3}[\s\S]+?\/{3}|\/(?!\s)[^[\/\n\\]*(?:(?:\\[\s\S]|\[[^\]\n\\]*(?:\\[\s\S][^\]\n\\]*)*])[^[\/\n\\]*)*\/)[imgy]{0,4}(?!\w)/, 0, '/'], ['com', /^\#(?:\##[^#][\s\S]*?###|.*)/, 0, '#'], ['typ', /^(?:0x[\da-f]+|[1-9]\d?r[\da-z]+|(?:\d+(?:\.\d+)?|\.\d+)(?:e[+-]?\d+)?[a-z]*)/i, 0, '0123456789.']], [['kwd', /^(?:t(?:ry|h(?:row|en)|ypeof)|f(?:or(?:own)?|inally|unction)|n(?:ew|ot)|c(?:ontinue|a(?:se|tch)|lass)|i(?:f|n(?:stanceof)?|mport(?:all)?|s(?:nt)?)|e(?:lse|xtends)|d(?:e(?:fault|lete|bugger)|o)|un(?:less|til)|o[fr]|return|break|while|switch|and)\b/], ['typ', /^(?:t(?:rue|his)|false|null|void|super)\b/]]), ['co']);
}).call(this);
