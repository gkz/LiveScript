...
var LiveScript, path, fs, util, prelude, each, breakList, ref$, parseOptions, generateHelp, nameFromPath, SourceNode, version, say, warn, die, p, pp, ppp, o, positional, e, toInsert, that, filename, slice$ = [].slice;
LiveScript = require('..');
path = require('path');
fs = require('fs');
util = require('util');
...
function compileScript(filename, input, base){
  ...
  try {
    ...
    if (o.compile && o.header) {
      t.output = new SourceNode(null, null, null, ["// Generated by LiveScript " + version + "\n", t.output]);
    }
    if (json || o.run) {
      LiveScript.emit('run', t);
      t.result = LiveScript.run(t.output.toString(), options, true);
    }
    if (json) {
    ...
    if (o.run) {
      switch (false) {
      case !json:
        process.stdout.write(t.output.toString());
        break;
      case !o.print:
    ...
    if (o.print || !filename) {
      say(t.output.toString().trimRight());
    } else {
...
function writeJS(source, js, input, base, json){
  ...
  function compileWithMap(){
    var mapPath, sourceName;
    mapPath = jsPath + ".map";
    sourceName = path.relative(path.dirname(mapPath), source);
    ...
    if (o.map === 'linked' || o.map === "debug") {
      js.code += '\n//# sourceMappingURL=' + path.relative(path.dirname(jsPath), mapPath) + '\n';
    } else {
        ...
        fs.writeFile(mapPath, js.map || '\n', function(e2){
          ...
          if (o.map === "debug") {
            fs.writeFile(mapPath + ".debug", js.debug || '\n', function(e3){
              if (o.watch) {
                util.log(source + " => " + jsPath + ", " + mapPath + "[.debug]");
              }
            });
          } else {
            if (o.watch) {
              util.log(source + " => " + jsPath + ", " + mapPath);
            }
          }
        });
      } else {
