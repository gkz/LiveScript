#!/usr/bin/env node
var o, $args, argv, that, filename, __join = [].join, __bind = function(me, fn){ return function(){ return fn.apply(me, arguments) } }, __clone = function(it){
...
function compileScripts(){
  ...
  function walk(source, base, top){
    return fshoot('stat', source, function(stats){
      if (stats.isDirectory()) {
        return fshoot('readdir', source, function(files){
...
function writeJS(source, js, base){
  ...
  filename = path.basename(source).replace(/(?:(\.\w+)?\.\w+)?$/, function(_, $1){
    return $1 || '.js';
  });
...
function repl(){
  var repl, code, cont, stdin, reset, prompt, vm, ctx, _ref;
  code = '';
  cont = false;
  stdin = process.openStdin();
  repl = require('readline').createInterface(stdin, process.stdout, __bind(_ref = (_ref = __clone(require('repl').REPLServer.prototype), _ref.ctx = ctx, _ref.commands = [], _ref), _ref.complete));
  reset = function(){
    repl.line = code = '';
    return repl.prompt();
  };
  ...
  repl.on('line', function(it){
    ...
    try {
      if (o.compile) {
        ...
        _ = vm.runInNewContext(Coco.compile(code, {
          bare: true,
          repl: true
        }), ctx, 'repl');
        _ === void 8 || pp(ctx._ = _);
