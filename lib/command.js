...
function compileScript(filename, input, base){
  ...
  try {
    ...
    if (o.compile && o.header) {
      t.output = new SourceNode(1, 0, filename, ["// Generated by LiveScript " + version + "\n", t.output]);
    }
    if (json || o.run) {
...
function writeJS(source, js, base, json){
  ...
  function compileWithMap(){
    var mapPath;
    mapPath = jsPath + ".map";
    js = new SourceNode(1, 0, null, ["//# sourceMappingURL=" + path.relative(path.dirname(jsPath), mapPath) + "\n", js]);
    js.setFile(path.relative(path.dirname(jsPath), source));
