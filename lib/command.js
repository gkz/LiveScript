#!/usr/bin/env node
var argv, stdin, stdout, o, that, $args, filename, __join = [].join;
argv = process.argv, stdin = process.stdin, stdout = process.stdout;
global.Coco = require('./coco');
global.fs = require('fs');
global.path = require('path');
global.util = require('util');
global.say = function(it){
  return stdout.write(it + '\n');
};
global.warn = function(it){
  return stderr.write(it + '\n');
};
...
default:
  ...
  o.stdin || (o.stdin = stdin.readable);
  $args = o.$args;
...
function repl(){
  var repl, code, cont, reset, prompt, vm, server, _ttyWrite, _ref;
  argv[1] = 'repl';
  code = '';
  cont = false;
