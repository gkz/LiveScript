var oparser, o, sources, compileScripts, compileScript, compileStdin, watch, writeJs, printTokens, repl, forkNode, help, version, __importAll = function(obj, src){ for (var key in src) obj[key] = src[key]; return obj }, __slice = [].slice, __bind = function(me, fn){ return function(){ return fn.apply(me, arguments) } };
...
compileScripts = function(){
  ...
  compile = function(source, base, top){
        ...
        if (stats.isDirectory()) {
          return fs.readdir(source, function(err, files){
            ...
            for (_i = 0, _len = files.length; _i < _len; ++_i) {
              file = files[_i];
              compile(path.join(source, file), base);
            }
          });
  ...
  for (_i = 0, _len = (_ref = sources).length; _i < _len; ++_i) {
    source = _ref[_i];
    _results.push(compile(source, path.normalize(source), true));
  }
  return _results;
};
