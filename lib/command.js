var argv, o, $args, that, filename, __join = [].join, __replace = ''.replace;
argv = process.argv;
...
function repl(){
  ...
  code = repl.infunc ? '  ' : '';
  cont = false;
  readline = require('readline').createInterface(process.openStdin(), process.stdout);
  reset = function(){
    readline.line = code = '';
    readline.prompt();
  };
  ...
  readline.on('line', function(it){
    ...
    try {
      if (o.compile) {
        ...
        for (name in lsv = (__ref = LiveScript.savedScope) != null ? __ref.variables : void 8) {
          if ('__' === name.slice(0, 2)) {
            delete lsv[name];
            global[__replace.call(name, '.', '')] = void 8;
          }
        }
      }
    } catch (e) {
