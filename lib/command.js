...
var LiveScript, path, fs, util, prelude, each, breakList, ref$, parseOptions, generateHelp, nameFromPath, SourceNode, version, say, warn, die, p, pp, ppp, o, positional, e, validMapValues, toInsert, that, filename, slice$ = [].slice;
LiveScript = require('..');
path = require('path');
fs = require('fs');
util = require('util');
...
function compileScript(filename, input, base){
  ...
  try {
    ...
    if (run) {
      ...
      o.runContext == null && (o.runContext = {});
      t.result = Function(t.output.toString()).call(o.runContext);
      switch (false) {
      case !json:
