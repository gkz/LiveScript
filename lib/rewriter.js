(function(){
  ...
  addImplicitParentheses = function(tokens){
    ...
    while (token = tokens[++i]) {
      ...
      if (!(obj || prev.spaced && ((tag === "(" || tag === "[" || tag === "{" || tag === "..." || tag === "IDENTIFIER" || tag === "THISPROP" || tag === "STRNUM" || tag === "LITERAL" || tag === "THIS" || tag === "UNARY" || tag === "CREMENT" || tag === "FUNCTION" || tag === "IF" || tag === "TRY" || tag === "SWITCH" || tag === "CLASS" || tag === "SUPER") || (tag === "PARAM_START" || tag === "FUNC_ARROW") && ((_ref = tokens[i - 2]) != null ? _ref[0] : void 8) !== 'FUNCTION' || tag === 'PLUS_MINUS' && !(token.spaced || token.eol)))) {
        continue;
      }
      seenSingle = false;
      if (soak = prev[0] === '?') {
