(function(){
  ...
  addImplicitBraces = function(tokens){
    var go, i, idx, ok, paren, stack, start, tag, tok, token, _ref;
    go = function(token, i){
  ...
  addImplicitParentheses = function(tokens){
    ...
    ok = function(token, i){
      ...
      return (tag === "POST_IF" || tag === "FOR" || tag === "WHILE" || tag === "BY" || tag === "TO" || tag === "CASE" || tag === "DEFAULT" || tag === "TERMINATOR") || tag === 'INDENT' && (pre !== "FUNC_ARROW" && pre !== "{" && pre !== "[" && pre !== ",") && ((_ref = tokens[i - 2]) != null ? _ref[0] : void 0) !== 'CLASS' && !((post = tokens[i + 1]) && post.generated && post[0] === '{');
    };
    go = function(token, i){
      if (token[0] === 'OUTDENT') {
        ++i;
  ...
  rewriteClosingParens = function(tokens){
    ...
    while (token = tokens[++i]) {
      ...
      pos = ((_ref = tokens[i + 2]) != null ? _ref[0] : void 0) === start ? (stack.push(stoken), i + 3) : i;
