...
function addImplicitParentheses(tokens){
  var i, token, tag, prev, seenCase, seenSwitch, seenClass, seenSingle, soak, _ref;
  i = 0;
  while (token = tokens[++i]) {
    tag = token[0];
    ...
    if (!((tag === '(' || tag === '[' || tag === '{' || tag === '...' || tag === 'IDENTIFIER' || tag === 'THISPROP' || tag === 'THIS' || tag === 'STRNUM' || tag === 'LITERAL' || tag === 'RANGE' || tag === 'UNARY' || tag === 'CREMENT' || tag === 'IF' || tag === 'TRY' || tag === 'CLASS' || tag === 'SWITCH' || tag === 'FUNCTION' || tag === 'SUPER') || tag === '+-' && !(token.spaced || token.eol) || (tag === 'PARAM(' || tag === 'FUNC_ARROW') && ((_ref = tokens[i - 2]) != null ? _ref[0] : void 8) !== 'FUNCTION')) {
      continue;
    }
    seenSingle = seenClass = seenSwitch = seenCase = false;
    if (soak = prev[0] === '?') {
      tokens.splice(--i, 1);
    }
