(function(){
  ...
  addImplicitBraces = function(tokens){
    ...
    ok = function(token, i){
      ...
      if (tag === ',') {
        ...
        return one !== 'HERECOMMENT' && ':' !== ((_ref = tokens[one === '('
  ...
  addImplicitParentheses = function(tokens){
    ...
    while (token = tokens[++i]) {
      ...
      if (!(token.argument || (tag === '(' || tag === '[' || tag === '{' || tag === '...' || tag === 'IDENTIFIER' || tag === 'THISPROP' || tag === 'STRNUM' || tag === 'LITERAL' || tag === 'THIS' || tag === 'UNARY' || tag === 'CREMENT' || tag === 'FUNCTION' || tag === 'IF' || tag === 'TRY' || tag === 'SWITCH' || tag === 'CLASS' || tag === 'SUPER') || tag === 'PLUS_MINUS' && !(token.spaced || token.eol) || (tag === 'PARAM_START' || tag === 'FUNC_ARROW') && ((_ref = tokens[i - 2]) != null ? _ref[0] : void 8) !== 'FUNCTION')) {
        continue;
      }
