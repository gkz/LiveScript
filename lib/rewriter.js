...
function addImplicitParentheses(tokens){
  var i, brackets, token, doblock, tpair, tag, prev, postfn, soak, seenSwitch, skipBlock, _ref, _ref2;
  i = -1;
  brackets = [];
  while (token = tokens[++i]) {
...
function expandLiterals(tokens){
  var i, token, num, sig, ts, lno, to, n, pre, _step, _ref;
  i = -1;
  while (token = tokens[++i]) {
    switch (token[0]) {
    case 'STRNUM':
    ...
    case 'RANGE':
      ...
      break;
    case 'INDENT':
      if ((pre = tokens[i - 1]) && (pre[1] === 'new' || pre[0] === 'FUNCTION')) {
...
ARG = CHAIN.concat(['->', '...', 'UNARY', 'CREMENT', 'PARAM(', 'FUNCTION', 'IF', 'SWITCH', 'TRY', 'CLASS', 'SUPER', 'RANGE', 'LABEL']);