var BALANCED_PAIRS, INVERSES, OPENER, CLOSER, left, rite, CHAIN, ARG, _i, _len, _ref, __indexOf = [].indexOf || function(x){
...
function removeTerminators(tokens){
  ...
  while (that = tokens[++i]) {
    ...
    if ((_ref = that[0]) === 'ELSE' || _ref === 'CASE' || _ref === 'DEFAULT' || _ref === 'CATCH' || _ref === 'FINALLY' || __indexOf.call(CLOSER, _ref) >= 0) {
      tokens.splice(i - 1, 1);
    }
  }
}
...
function addImplicitBraces(tokens){
  ...
  while (token = tokens[++i]) {
    if (':' !== (tag = token[0])) {
      switch (false) {
      case __indexOf.call(CLOSER, tag) < 0:
        start = stack.pop();
        break;
      case __indexOf.call(OPENER, tag) < 0:
...
function addImplicitParentheses(tokens){
  var i, brackets, token, doblock, endi, tpair, tag, prev, postfn, seenSwitch, skipBlock, _ref, _ref2;
  i = -1;
  brackets = [];
  while (token = tokens[++i]) {
    doblock = false;
...
function detectEnd(tokens, i, ok, go){
  ...
  while (token = tokens[i]) {
    ...
    tag = token[0];
    if (__indexOf.call(OPENER, tag) >= 0) {
      ++levels;
    } else if (__indexOf.call(CLOSER, tag) >= 0) {
      --levels;
    }
    ++i;
  }
...
for (_i = 0, _len = BALANCED_PAIRS.length; _i < _len; ++_i) {
  ...
  OPENER.push(INVERSES[rite] = left);
  CLOSER.push(INVERSES[left] = rite);
}
