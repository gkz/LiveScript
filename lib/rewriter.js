(function(){
  var BALANCED_PAIRS, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_FUNC, INVERSES, addImplicitBraces, addImplicitIndentation, addImplicitParentheses, closeOpenings, detectEnd, ensureBalance, indentation, indexOfPair, left, removeLeadingTerminators, removeMidExpressionTerminators, rewriteClosingParens, rite, tagPostfixConditionals, _i, _len, _ref, __indexOf = Array.prototype.indexOf || function(x){
  ...
  addImplicitParentheses = function(tokens){
    var classLine, go, i, next, obj, ok, prev, seenSingle, soak, tag, token, _ref;
    seenSingle = false;
    ok = function(token, i){
    ...
    while (token = tokens[++i]) {
      ...
      switch (tag) {
      case 'INDENT':
        obj = !classLine && (next = tokens[i + 1]) && next.generated && next[0] === '{';
        /* fallthrough */
      case "TERMINATOR":
      case "OUTDENT":
        classLine = false;
      ...
      if (!(prev && (prev.call || (_ref = prev[0], __indexOf.call(IMPLICIT_FUNC, _ref) >= 0)))) {
        continue;
      }
      if (!(obj || prev.spaced && ((tag === "(" || tag === "[" || tag === "{" || tag === "IDENTIFIER" || tag === "THISPROP" || tag === "STRNUM" || tag === "LITERAL" || tag === "THIS" || tag === "UNARY" || tag === "CREMENT" || tag === "FUNCTION" || tag === "IF" || tag === "TRY" || tag === "SWITCH" || tag === "CLASS" || tag === "SUPER" || tag === "...") || (tag === "PARAM_START" || tag === "FUNC_ARROW") && ((_ref = tokens[i - 2]) != null ? _ref[0] : void 8) !== 'FUNCTION' || tag === 'PLUS_MINUS' && !(token.spaced || token.eol)))) {
        continue;
      }
      seenSingle = false;
      if (soak = prev[0] === '?') {
  ...
  addImplicitIndentation = function(tokens){
    var go, i, indent, next, ok, outdent, tag, token, _ref, _ref2;
    ok = function(token, i){
      var tg;
      tg = token[0];
    ...
    while (token = tokens[++i]) {
      ...
      if (!(tag === 'THEN' || (next = (_ref = tokens[i + 1]) != null ? _ref[0] : void 8) !== 'INDENT' && ((tag === "FUNC_ARROW" || tag === "DEFAULT" || tag === "TRY" || tag === "FINALLY") || tag === 'ELSE' && next !== 'IF'))) {
        continue;
      }
