(function(){
  ...
  addImplicitParentheses = function(tokens){
    ...
    while (token = tokens[++i]) {
      ...
      if (!(callObject || (prev != null ? prev.spaced : void 0) && (prev.call || (_ref = prev[0], __indexOf.call(IMPLICIT_FUNC, _ref) >= 0)) && ((tag === "(" || tag === "[" || tag === "{" || tag === "IDENTIFIER" || tag === "THISPROP" || tag === "STRNUM" || tag === "LITERAL" || tag === "THIS" || tag === "UNARY" || tag === "CREMENT" || tag === "FUNCTION" || tag === "IF" || tag === "TRY" || tag === "SWITCH" || tag === "CLASS" || tag === "SUPER") || (tag === "PARAM_START" || tag === "FUNC_ARROW") && ((_ref = tokens[i - 2]) != null ? _ref[0] : void 0) !== 'FUNCTION' || tag === 'PLUS_MINUS' && !(token.spaced || token.eol)))) {
        continue;
      }
