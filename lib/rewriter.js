var OPENERS, CLOSERS, INVERSES, CHAIN, ARG, __indexOf = [].indexOf || function(x){
...
function removeTerminators(tokens){
  ...
  while (that = tokens[++i]) {
    ...
    if ((_ref = that[0]) === 'ELSE' || _ref === 'CASE' || _ref === 'DEFAULT' || _ref === 'CATCH' || _ref === 'FINALLY' || __indexOf.call(CLOSERS, _ref) >= 0) {
      tokens.splice(i - 1, 1);
    }
  }
}
...
function expandLiterals(tokens){
  var i, token, sig, ts, lno, to, by, n, that, _ref, _step;
  i = -1;
  while (token = tokens[++i]) {
    switch (token[0]) {
    case 'STRNUM':
    ...
    case 'RANGE':
      ts = [];
      lno = token[2];
      to = tokens[i + 1][1] - (token.op === 'til' && 1e-15);
      by = ((_ref = tokens[i + 2]) != null ? _ref[0] : void 8) === 'RANGE_BY' && ((_ref = tokens[i + 3]) != null ? _ref[1] : void 8);
      for (n = +token[1], _step = +by || 1; _step < 0 ? n >= to : n <= to; n += _step) {
