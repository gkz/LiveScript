(function(){
  var detectEnd, removeLeadingTerminators, removeMidExpressionTerminators, closeOpenings, addImplicitBraces, addImplicitParentheses, addImplicitIndentation, tagPostfixConditionals, ensureBalance, rewriteClosingParens, indexOfPair, BALANCED_PAIRS, INVERSES, EXPR_START, EXPR_END, left, rite, EXPR_CLOSE, _i, _len, _ref, __indexOf = [].indexOf || function(x){
  ...
  removeMidExpressionTerminators = function(tokens){
    ...
    i = 0;
    while (token = tokens[++i]) {
      if (tokens[i - 1][0] === 'TERMINATOR' && (_ref = token[0], __indexOf.call(EXPR_CLOSE, _ref) >= 0)) {
        tokens.splice(i - 1, 1);
      }
    }
    return tokens;
  ...
  addImplicitParentheses = function(tokens){
    ...
    while (token = tokens[++i]) {
      ...
      if (!(prev = tokens[i - 1]).spaced) {
        if (tag === '?') {
          token.call = true;
        } else if (tag === '{' && (prev[0] === '}' || callable(prev[0]))) {
    ...
    function callable(it){
      var _ref;
      return (it === 'IDENTIFIER' || it === 'THISPROP' || it === 'SUPER' || it === 'THIS' || it === ')' || it === ')CALL' || it === ']' || it === ']INDEX') || it === 'STRNUM' && ((_ref = tokens[i - 2]) != null ? _ref[0] : void 8) === 'DOT';
    }
    function ok(token, i){
  ...
  addImplicitIndentation = function(tokens){
    ...
    while (token = tokens[++i]) {
      ...
      case ',' !== (next = tokens[i + 1][0]):
        go(0, ++i);
        break;
      case !(',' === ((_ref = tokens[i + 2]) != null ? _ref[0] : void 8) && (next === 'IDENTIFIER' || next === 'THISPROP' || next === 'STRNUM' || next === 'LITERAL' || next === 'THIS' || next === 'SUPER' || next === 'FUNC_ARROW')):
        go(0, i += 2);
        break;
  ...
  BALANCED_PAIRS = [['(', ')'], ['[', ']'], ['{', '}'], ['CALL(', ')CALL'], ['INDEX[', ']INDEX'], ['PARAM(', ')PARAM'], ['INDENT', 'DEDENT']];
  INVERSES = {};
  EXPR_START = [];
  EXPR_END = [];
