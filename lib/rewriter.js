...
function addImplicitIndentation(tokens){
  ...
  while (token = tokens[++i]) {
    tag = token[0];
    if (tag !== 'THEN' && tag !== 'FUNC_ARROW' && tag !== 'ELSE' && tag !== 'DEFAULT' && tag !== 'TRY' && tag !== 'CATCH' && tag !== 'FINALLY') {
      continue;
    }
  ...
  function go(token, i){
    return tokens.splice(tokens[i - 1][0] === ',' ? i - 1 : i, 0, (dedent[2] = token[2], dedent));
  }
}
...
function addImplicitParentheses(tokens){
  var i, brackets, token, tag, prev, doblock, tpair, seenSwitch, skipBlock, soak, _ref;
  i = 0;
  brackets = [];
  while (token = tokens[++i]) {
    tag = token[0];
  ...
  function ok(token, i){
    ...
    case 'INDENT':
      ...
      return (_ref = pre[0]) !== '{' && _ref !== '[' && _ref !== ',' && _ref !== 'FUNC_ARROW' && _ref !== 'ELSE' && _ref !== 'TRY' && _ref !== 'CATCH' && _ref !== 'FINALLY' && _ref !== 'DEFAULT' && _ref !== 'UNARY';
    case 'TERMINATOR':
    case 'POST_IF':
    case 'FOR':
    case 'WHILE':
...
ARG = ['(', '[', '{', '...', 'IDENTIFIER', 'STRNUM', 'LITERAL', 'RANGE', 'PARAM(', 'FUNC_ARROW', 'FUNCTION', 'UNARY', 'CREMENT', 'IF', 'TRY', 'CLASS', 'SWITCH', 'LET', 'WITH', 'SUPER'];