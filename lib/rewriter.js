...
exports.able = able;
function rewrite(it){
  removeLeadingTerminators(it);
  removeMidExpressionTerminators(it);
  closeCalls(it);
...
function addImplicitParentheses(tokens){
  var i, token, tag, prev, seenSwitch, skipBlock, soak;
  i = 0;
  while (token = tokens[++i]) {
    tag = token[0];
    ...
    if (!((tag === '(' || tag === '[' || tag === '{' || tag === '...' || tag === 'IDENTIFIER' || tag === 'THISPROP' || tag === 'THIS' || tag === 'STRNUM' || tag === 'LITERAL' || tag === 'PARAM(' || tag === 'FUNC_ARROW' || tag === 'FUNCTION' || tag === 'RANGE' || tag === 'SUPER' || tag === 'UNARY' || tag === 'CREMENT' || tag === 'IF' || tag === 'TRY' || tag === 'CLASS' || tag === 'SWITCH') || tag === '+-' && !(token.spaced || token.eol) || (skipBlock = seenSwitch = false))) {
      continue;
    }
    if (soak = prev[0] === '?') {
      tokens.splice(--i, 1);
