...
exports.rewrite = function(it){
  return rewriteClosingParens(ensureBalance(addImplicitParentheses(addImplicitBraces(tagPostfixConditionals(addImplicitIndentation(closeCalls(removeMidExpressionTerminators(removeLeadingTerminators(it)))))))));
};
...
function addImplicitParentheses(tokens){
  ...
  function ok(token, i){
    ...
    if (token.decall || !seenSingle && token.fromThen) {
      return true;
    }
    tag = token[0];
