(function(){
  var BALANCED_PAIRS, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_CALL, IMPLICIT_FUNC, INVERSES, left, rite, _i, _len, _ref, __indexOf = Array.prototype.indexOf || function(item){
  ...
  exports.Rewriter.prototype.addImplicitBraces = function(){
    var action, condition, i, idx, paren, stack, start, tag, tok, token, tokens, _ref;
    tokens = this.tokens;
    condition = function(token, i){
      var one, tag, _ref, _ref2;
      one = (_ref = tokens[i + 1]) != null ? _ref[0] : void 0;
      if ('HERECOMMENT' === one || 'HERECOMMENT' === ((_ref = tokens[i - 1]) != null ? _ref[0] : void 0)) {
        return false;
      }
      tag = token[0];
  ...
  exports.Rewriter.prototype.addImplicitParentheses = function(){
    var action, callObject, classLine, condition, i, next, prev, seenSingle, tag, token, tokens, _ref;
    ...
    condition = function(token, i){
      ...
      return (tag === "POST_IF" || tag === "FOR" || tag === "WHILE" || tag === "WHEN" || tag === "BY" || tag === "TO" || tag === "CASE" || tag === "DEFAULT" || tag === "TERMINATOR") || tag === 'INDENT' && (pre !== "FUNCTION" && pre !== "{" && pre !== "[" && pre !== ",") && ((_ref = tokens[i - 2]) != null ? _ref[0] : void 0) !== 'CLASS' && !((post = tokens[i + 1]) && post.generated && post[0] === '{');
    };
    action = function(token, i){
      if (token[0] === 'OUTDENT') {
        ++i;
    ...
    while (token = tokens[++i]) {
      ...
      seenSingle = false;
      this.detectEnd(i, condition, action);
      if (prev[0] === '?') {
        prev[0] = 'FUNC_EXIST';
      }
    }
  ...
  exports.Rewriter.prototype.rewriteClosingParens = function(){
    var debt, end, i, inv, key, stack, start, stoken, tag, tok, token, tokens, _ref;
    stack = [];
    debt = {};
    for (key in INVERSES) {
      debt[key] = 0;
