(function(){
  ...
  addImplicitParentheses = function(tokens){
    ...
    while (token = tokens[++i]) {
      ...
      if (!(callObject || (prev != null ? prev.spaced : void 0) && (prev.call || (_ref = prev[0], __indexOf.call(IMPLICIT_FUNC, _ref) >= 0)) && ((tag === "IDENTIFIER" || tag === "THISPROP" || tag === "STRNUM" || tag === "LITERAL" || tag === "THIS" || tag === "UNARY" || tag === "CREMENT" || tag === "FUNCTION" || tag === "IF" || tag === "TRY" || tag === "SWITCH" || tag === "CLASS" || tag === "[" || tag === "(" || tag === "{") || (tag === "PARAM_START" || tag === "FUNC_ARROW") && ((_ref = tokens[i - 2]) != null ? _ref[0] : void 0) !== 'FUNCTION' || tag === 'PLUS_MINUS' && !(token.spaced || token.eol)))) {
        continue;
      }
  ...
  ensureBalance = function(tokens){
    var close, level, levels, olines, open, tag, token, _i, _j, _len, _len2, _ref;
    levels = {};
    olines = {};
    for (_i = 0, _len = tokens.length; _i < _len; ++_i) {
      token = tokens[_i];
      ...
      for (_j = 0, _len2 = BALANCED_PAIRS.length; _j < _len2; ++_j) {
        _ref = BALANCED_PAIRS[_j], open = _ref[0], close = _ref[1];
        levels[open] |= 0;
        if (tag === open) {
          if (levels[open]++ === 0) {
            olines[open] = token[2];
