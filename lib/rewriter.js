...
exports.rewrite = function(it){
  return expandPrimitives(rewriteClosingParens(ensureBalance(addImplicitParentheses(addImplicitBraces(tagPostfixConditionals(addImplicitIndentation(closeCalls(removeMidExpressionTerminators(removeLeadingTerminators(it))))))))));
};
...
function expandPrimitives(tokens){
  ...
  while (token = tokens[++i]) {
    ...
    case 'STRNUM':
      ...
      } else if (!last.spaced && last[0] !== 'STRNUM' && able(tokens, i)) {
