(function(){
  ...
  addImplicitBraces = function(tokens){
    ...
    ok = function(token, i){
      ...
      tag = token[0];
      return tag === 'OUTDENT' || tag === ',' && (one !== "IDENTIFIER" && one !== "STRNUM" && one !== "THISPROP" && one !== "TERMINATOR" && one !== "OUTDENT" && one !== "(") || tag === 'TERMINATOR' && (one === '(' ? ((_ref = tokens[1 + indexOfPair(tokens, i + 1)]) != null ? _ref[0] : void 8) !== ':' : (_ref = (_ref2 = tokens[i + 2]) != null ? _ref2[0] : void 8) !== ":" && _ref !== "...");
    };
    stack = [];
    i = -1;
    while (token = tokens[++i]) {
      ...
      if (((_ref = tokens[idx - 2]) != null ? _ref[0] : void 8) === 'HERECOMMENT') {
        idx -= 2;
      }
      tok = ['{', '{', token[2]];
      tok.generated = true;
