...
o = function(patterns, action, options){
  ...
  action && (action = action === ditto
    ? last
    : (action + "").replace(/^function\s*\(\)\s*\{\s*return\s*([\s\S]*);\s*\}/, '$$$$ = L(@1, @' + patterns.length + ',$1);').replace(/\b(?!Er)[A-Z][\w.]*/g, 'yy.$&').replace(/(\.L\()\s*(\d+\s*\,)\s*(\d+\s*\,)?/g, function(_, a, b, c){
