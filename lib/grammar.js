(function(){
  ...
  grammar = {
    ...
    Literal: [
      o('AlphaNumeric'), o('THIS', function(){
        ...
        return $1 === 'void' ? new Op('void', new Literal(0)) : new Literal($1);
      })
    ],
    Assign: [
    ...
    Operation: [
      o('UNARY Expression', function(){
        ...
        return new Import($1, $3, true);
      }), o('Expression IMPORT ALL Expression', function(){
        return new Import($1, $4);
  ...
  operators = [['left', '.', '?.', '.='], ['left', 'CALL_START', 'CALL_END'], ['nonassoc', '++', '--'], ['left', '?'], ['right', 'UNARY'], ['left', 'MATH'], ['left', '+', '-'], ['left', 'SHIFT'], ['left', 'RELATION', 'IMPORT'], ['left', 'COMPARE'], ['left', 'LOGIC'], ['nonassoc', 'INDENT', 'OUTDENT'], ['right', '=', ':', 'COMPOUND_ASSIGN', 'RETURN'], ['right', 'WHEN', 'LEADING_WHEN', 'FORIN', 'FOROF', 'FROM', 'TO', 'BY', 'THROW', 'IF', 'UNLESS', 'ELSE', 'FOR', 'WHILE', 'LOOP', 'SUPER', 'CLASS', 'EXTENDS'], ['right', 'POST_IF']];
  tokens = [];
  for (name in grammar) {
