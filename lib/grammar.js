...
o = function(patterns, action, options){
  ...
  action && (action = unwrap.exec(action)[1].replace(/\b(?:(?!Er)[A-Z][\w.]*|mix\b)/g, 'yy.$&').replace(/\.L\(/g, '$&yylineno, '));
...
bnf = {
  ...
  Line: [
    o('Expression'), o('COMMENT', function(){
      ...
      return L(new Throw(Literal("Error('stub code executed')")));
    })
  ],
  Body: [
    o('Line', function(){
  ...
  ThisProperty: [
    o('THISPROP', function(){
      return L(Value(Literal('this'), [Index(Key($1))], true));
    })
  ],
  Parenthetical: [
    o('(        Body        )', function(){
