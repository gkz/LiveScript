...
operators = [['left', 'PIPE', 'POST_IF', 'FOR', 'WHILE'], ['right', 'BACKPIPE'], ['right', ',', 'ASSIGN', 'HURL', 'EXTENDS', 'INDENT', 'SWITCH', 'CASE', 'TO', 'BY', 'LABEL'], ['right', 'CONCAT'], ['right', 'LOGIC'], ['left', 'BITWISE'], ['right', 'COMPARE'], ['left', 'RELATION'], ['left', 'SHIFT', 'IMPORT'], ['left', '+-'], ['left', 'MATH'], ['right', 'UNARY'], ['right', 'POWER', '^'], ['right', 'COMPOSE'], ['nonassoc', 'CREMENT'], ['right', 'BACKTICK']];
tokens = (function(){
  var __ref, __i, __ref1, __len, __results = [];
  ...
  function __fn(){
    ...
    for (__i = 0, __len = (__ref = alt[0]).length; __i < __len; ++__i) {
      token = __ref[__i];
      if (!(token in bnf)) {
