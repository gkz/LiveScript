...
o = function(patterns, action, options){
  ...
  action && (action = action === ditto
    ? last
    : (action + "").replace(/^function\s*\(\)\s*\{\s*return\s*([\s\S]*);\s*\}/, function(_, a){
      return '$$ = ' + (a.slice(0, 2) === 'L('
        ? '('
        : 'L(@1, @' + patterns.length + ',') + a + ');';
    }).replace(/\b(?!Er)(?!String)[A-Z][\w.]*/g, 'yy.$&').replace(/(\.L\()\s*(\d+\s*\,)\s*(\d+\s*\,)?/g, function(_, a, b, c){
...
bnf = {
  ...
  Line: [
    o('Expression'), o('Expression Block', function(){
      ...
      return Call.back($2, $6, $5.charAt(1) === '~', $5.length === 3, /\*/.test($5));
    }), o('COMMENT', function(){
  ...
  LoopHead: [
    ...
    }), o('FOR ID FROM Expression CASE Expression', function(){
      return new ForOf({
        item: $2,
        source: $4,
        guard: $6
