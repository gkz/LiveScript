(function(){
  var Access, Arr, Assign, Call, Case, Class, Code, Comment, Existence, Expressions, Extends, For, IDENTIFIER, If, Import, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, METHOD_DEF, Node, Obj, Of, Op, Param, Parens, Return, SIMPLENUM, Scope, Splat, Super, Switch, TAB, Throw, Try, UTILITIES, Value, While, lastNonComment, multident, utility, __extends = function(sub, sup){
      ...
      for (_i = 0, _len = (_ref = this.expressions).length; _i < _len; ++_i) {
        ...
        if (top) {
          ...
          if (!node.isStatement(o)) {
            code = o.indent + code + node.terminater;
          }
        } else {
  ...
  exports.Call = Call = (function(){
    ...
    _ref.compileNode = function(o){
      ...
      if (!(fun = (this.callee.base || this.callee) instanceof Code)) {
  ...
  exports.Class = Class = (function(){
    ...
    _ref.children = ["title", "parent", "code"];
    _ref.compileNode = function(o){
      ...
      for (i = 0, _len = (_ref = exps = this.code.body.expressions).length; i < _len; ++i) {
        node = _ref[i];
        if (node.isObject()) {
    ...
    _ref.compileNode = function(o){
      ...
      if (this.bound === '_this') {
        if (this.ctor) {
          ...
          Node.prototype.traverseChildren.call(this, function(it){
            if (it instanceof Return) {
              it.expression || (it.expression = Literal('_this'));
            }
            return null;
          });
  ...
  exports.Op = Op = (function(){
    ...
    _ref.isChain = function(){
      return this.first instanceof Op && COMPARER.test(this.op) && COMPARER.test(this.first.op);
    };
    _ref.invert = function(){
      var fst, op, _ref;
      if (EQUALITY.test(op = this.op) && !this.isChain()) {
      ...
      if (op === '!' && (fst = this.first.unwrap()) instanceof Op && ((_ref = fst.op) === "!" || _ref === "in" || _ref === "instanceof" || _ref === "<" || _ref === ">" || _ref === "<=" || _ref === ">=")) {
        return fst;
      }
      return Op('!', this);
    };
  ...
  exports.For = For = (function(){
    ...
    _ref.compileNode = function(o){
      ...
      if (this.index instanceof Node && !(this.index = this.index.unwrap().value)) {
