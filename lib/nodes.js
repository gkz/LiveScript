var Node, Negatable, Block, Atom, Literal, Var, Key, Index, Value, Call, Clone, List, Obj, Arr, Op, Assign, Import, Of, Existence, Fun, Class, Super, Parens, Splat, Statement, Throw, Return, While, For, Try, Switch, Case, If, JS, Util, UTILITIES, LEVEL_TOP, LEVEL_PAREN, LEVEL_LIST, LEVEL_COND, LEVEL_OP, LEVEL_ACCESS, PREC, TAB, IDENTIFIER, _ref, __import = function(obj, src){
...
Node = (function(){
  ...
  _proto.compileClosure = function(o){
    ...
    return Parens(Value(fun, [(_ref = Call(args), _ref.method = method, _ref)]), true).compileNode(o);
  };
  ...
  _proto.asKey = THIS;
  _proto.asArr = THIS;
  _proto.varName = String;
  _proto.selections = NO;
  _proto.getCall = NO;
  _proto.invert = function(){
    return Op('!', this);
  };
  _proto.makeReturn = function(arref){
    if (arref) {
  ...
  _proto.stringify = function(space){
...
exports.Block = Block = (function(_super){
  ...
  _proto.compileRoot = function(options){
    var o, bare, first, shebang, node, i, code, _ref, _ref2, _ref3;
    o = __importAll({
      level: LEVEL_TOP,
      scope: this.scope = Scope.root = new Scope
    }, options);
    o.indent = this.tab = (bare = (_ref = o.bare, delete o.bare, _ref)) ? '' : TAB;
    shebang = (first = this.lines[0]) instanceof JS && ~first.code.lastIndexOf('#!', 0) && this.lines.shift().code;
...
exports.Value = Value = (function(_super){
  ...
  _proto.cacheReference = function(o){
    ...
    if (base.isComplex()) {
      ...
      base = Value(Assign(Var(ref), base));
  ...
  _proto.compileNode = function(o){
    ...
    if (that = this.unfoldAssign(o) || this.multiPick(o)) {
  ...
  _proto.unfoldSoak = function(o){
    ...
    for (i = 0, _len = (_ref = this.tails).length; i < _len; ++i) {
      ...
      if (node.soak) {
        node.soak = '';
        _ref2 = Value(this.head, this.tails.splice(0, i)).cacheReference(o), test = _ref2[0], this.head = _ref2[1];
        return If(JS("typeof " + test.compile(o, LEVEL_OP) + " == 'function'"), this, {
          soak: true
        });
      } else if (node.symbol === '?.') {
        node.symbol = '';
        _ref2 = Value(this.head, this.tails.splice(0, i)).unwrap().cache(o, true), test = _ref2[0], this.head = _ref2[1];
  ...
  _proto.unfoldAssign = function(o){
    ...
    for (i = 0, _len = (_ref = this.tails).length; i < _len; ++i) {
      ...
      if (that = index.assign) {
        index.assign = '';
        _ref2 = Value(this.head, this.tails.splice(0, i)).cacheReference(o), left = _ref2[0], this.head = _ref2[1];
        return _ref2 = Assign(left, this, that), _ref2.access = true, _ref2;
      }
    }
  };
  _proto.expandSplat = function(o){
    ...
    while (call = tails[++i]) {
      ...
      if (tails[i - 1] instanceof Index) {
        _ref = Value(this.head, tails.splice(0, i - 1)).cache(o, true), this.head = _ref[0], ref = _ref[1];
        i = 0;
      }
      call.args = [ref || Literal('null'), JS(args)];
    }
  };
  _proto.expandBind = function(o){
    var tails, i, index, args, call;
    tails = this.tails;
    i = -1;
    while (index = tails[++i]) {
      if (index.symbol !== '~.') {
      ...
      index.symbol = '';
      args = Value(this.head, tails.splice(0, i)).unwrap().cache(o, true);
      args[1] = Value(args[1], [tails.shift()]);
      call = Call.make(Util('bind'), args);
      this.head = this.newed ? Parens(call, true) : call;
      i = -1;
    }
  };
  _proto.expandStar = function(o){
    var tails, i, index, stars, sub, ref, temps, star, _ref, _i, _len;
    tails = this.tails;
    i = -1;
    while (index = tails[++i]) {
      if (index.args || index.stars || index.key instanceof Key) {
        continue;
      }
      stars = index.stars = [];
      ...
      _ref = Value(this.head, tails.splice(0, i)).unwrap().cache(o), sub = _ref[0], ref = _ref[1], temps = _ref[2];
      ...
      this.head = JS(sub.compile(o, LEVEL_ACCESS) + tails.shift().compile(o));
      if (temps) {
  ...
  _proto.compile = function(o){
    ...
    return (this.method || '') + "(" + (function(){
      ...
      for (_i = 0, _len = (_ref = this.args).length; _i < _len; ++_i) {
        a = _ref[_i];
        _results.push(a.compile(o, LEVEL_LIST));
      }
      return _results;
    }.call(this)).join(', ') + ")";
  };
  Call.make = function(callee, args){
    return Value(callee).add(Call(args));
  };
  Call.back = function(params, arrow, node){
    var call, args, i, a, _len;
    (call = node.getCall()) || (node = Value(node).add(call = Call()));
    args = call.args;
...
exports.Op = Op = (function(_super){
  ...
  function Op(op, first, second, post){
    var args, ps, method, i, p, node, _len, _ref, _i, _this = new _ctor;
    switch (op) {
    case 'of':
    ...
    case 'do':
      if (first instanceof Fun) {
        args = [];
        ps = first.params;
        if (ps.length && ps[0].op === '~') {
          args.push(ps.shift().first);
          method = '.call';
        }
        for (i = 0, _len = ps.length; i < _len; ++i) {
          p = ps[i];
          args.push(p.op === '=' && !p.logic ? (ps[i] = p.left, p.right) : p);
        }
      }
      return Parens(Value(first).add((_ref = Call(args), _ref.method = method, _ref)));
    case 'new':
      first.newed = true;
  ...
  _proto.compileExistence = function(o){
    var fsts, code;
    if (!o.level) {
      return Op('&&', Existence(this.first).invert(), this.second).compile(o);
    }
...
exports.Assign = Assign = (function(_super){
  ...
  _proto.transleft = function(o){
    var left, base, items, sub, ref, i, node, logic, key, val, tail, k, _ref, _len;
    left = this.left;
    if (left instanceof Clone) {
      ...
      left = Obj(items);
    } else if (left instanceof Value && (tail = (_ref = left.tails)[_ref.length - 1]) instanceof Index && tail.key.length) {
...
exports.Import = Import = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    if (this.all || !this.right.items) {
      return Call.make(Util("import" + (this.all || '')), [this.left, this.right]).compile(o);
    }
...
exports.Fun = Fun = (function(_super){
  ...
  _proto.compileNode = function(o){
    var pscope, sscope, scope, params, body, name, tab, code, that, args, asns, i, p, splatted, arg, dfv, ref, val, lines, wasEmpty, dic, a, _ref, _len, _i;
    pscope = o.scope;
    ...
    if (this.statement) {
      name || this.carp('nameless function');
...
exports.Class = Class = (function(_super){
  ...
  _proto.isCallable = YES;
  _proto.compileNode = function(o){
    var fun, title, lines, decl, name, lname, i, node, proto, ctor, args, clas, _len, _ref;
    fun = this.fun, title = this.title;
    lines = fun.body.lines;
    if (title) {
      decl = title.varName() || title.value;
    }
    name = decl || this.name;
    ...
    for (i = 0, _len = lines.length; i < _len; ++i) {
      ...
      if (node instanceof Obj) {
        ...
        if (ctor) {
          node.carp('more than one constructor in a class');
        }
        if ((ctor = node).bound) {
          proto = lname;
        }
...
exports.Super = Super = (function(_super){
  ...
  _proto.compile = function(o){
    ...
    fun || this.carp('super outside of a function');
    ...
    return this.carp('super in an anonymous function');
  };
  return Super;
}(Node));
exports.Parens = Parens = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    if (!(this.keep || this.newed)) {
      if ((it instanceof Atom || it instanceof Value || it instanceof Fun || it instanceof Parens) || it instanceof Op && o.level < LEVEL_OP + (PREC[it.op] || PREC.unary)) {
        return (it.front = this.front, it).compile(o, Math.max(o.level, LEVEL_PAREN));
      }
    }
    o.level = LEVEL_PAREN;
    if (it.isStatement(o)) {
...
exports.Splat = Splat = (function(_super){
  ...
  Splat.compileArray = function(o, list, apply){
    ...
    for (_i = 0, _len = (_ref = list.splice(index, 9e9)).length; _i < _len; ++_i) {
      ...
      code = node.compile(o, LEVEL_LIST);
      if (node instanceof Splat) {
        atoms.length && (atoms.length = !args.push("[" + atoms.join(', ') + "]"));
    ...
    if (atoms.length) {
      args.push("[" + atoms.join(', ') + "]");
    }
    base = index
      ? "[" + (function(){
        var _i, _ref, _len, _results = [];
        for (_i = 0, _len = (_ref = list).length; _i < _len; ++_i) {
          node = _ref[_i];
...
exports.For = For = (function(_super){
  ...
  _proto.pluckDirectCalls = function(o){
    ...
    return this.body.eachChild(dig = function(it){
      var call, fn, name, ref, _ref;
      if (!((call = it.getCall()) && it.tails.length === 1 && ((fn = it.head.unwrap()) instanceof Fun || fn instanceof Parens && (fn = fn.it) instanceof Fun) && fn.params.length === call.args.length - !!call.method)) {
        return it instanceof Fun || it instanceof For ? null : it.eachChild(dig);
      }
      if (_this.index) {
        fn.params.push((_ref = call.args)[_ref.length] = Var(_this.index));
      }
      if (name = _this.name) {
...
exports.If = If = (function(_super){
  ...
  If.unfoldSoak = function(o, parent, name){
    ...
    if (that = parent[name].unfoldSoak(o)) {
      ...
      return that.then = Value(parent), that;
    }
  };
  return If;
}(Node));
