...
Node = (function(){
  ...
  _proto.compileClosure = function(o){
    ...
    if (this.jumps()) {
      throw SE(this.compile((o.indent = '', o), LEVEL_TOP) + ' // cannot be an expression');
    }
  ...
  _proto.compile = function(o, level){
    ...
    case 'null':
      if ((level != null ? level : o.level) === LEVEL_ACCESS) {
        throw SE('invalid use of ' + this.value);
      }
      return val;
    }
    if (val.js) {
...
exports.Obj = Obj = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    for (_i = 0, _len = items.length; _i < _len; ++_i) {
      ...
      if (dic[0 + key]) {
        throw SE('duplicate property name: ' + key);
      }
    ...
    if (op === 'do') {
      ...
      if (first instanceof Fun) {
        ...
        for (i = 0, _len = params.length; i < _len; ++i) {
          p = params[i];
          args.push(p.op === '=' && !p.logic ? (params[i] = p.left, p.right) : p);
        }
      }
...
exports.Assign = Assign = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    if (left.items) {
      ...
      throw SE("\"" + this.show() + "\" cannot destructure");
    }
    if (this.logic) {
    ...
    if (this.op === ':') {
      if (left.isComplex()) {
        throw SE('invalid property name: ' + name);
      }
      return name + ': ' + val;
    }
    if (!left.isAssignable()) {
      o.indent = '';
      o.level = LEVEL_LIST;
      throw SE(this.left.compile(o) + ' // cannot be assigned');
    }
    if (IDENTIFIER.test(name)) {
      if (this.op === '=') {
        o.scope.declare(name);
      } else if (!o.scope.check(name, true)) {
        throw SE('assignment to undeclared variable: ' + name);
      }
    }
  ...
  _proto.destructArr = function(o, nodes, rite){
    ...
    for (i = 0, _len = nodes.length; i < _len; ++i) {
      ...
      if (node instanceof Splat) {
        if (ivar) {
          throw SE('multiple splats in an assignment');
        }
        len = nodes.length;
  ...
  _proto.destructObj = function(o, nodes, rite){
    var node, splat, dfv, key, lr, val, _i, _len, _ref, _results = [];
    for (_i = 0, _len = nodes.length; _i < _len; ++_i) {
      node = nodes[_i];
      if (splat = node instanceof Splat) {
        node = node.it;
      }
      if (dfv = node.hasDefault()) {
        dfv = node;
        node = node.first;
      }
  ...
  _proto.transleft = function(o){
    var left, base, items, sub, ref, i, node, dfv, key, val, k, _ref, _len, _ref2;
    left = this.left;
    if (left.mixin) {
...
exports.Fun = Fun = (function(_super){
  ...
  _proto.compileNode = function(o){
    var pscope, sscope, scope, params, body, name, tab, code, that, args, asns, i, prm, p, splatted, arg, dfv, ref, val, exps, wasEmpty, dic, a, _ref, _len, _i;
    pscope = o.scope;
    ...
    if (this.bound === '_this') {
      ...
      } else if (that = (_ref = sscope.method) != null ? _ref.bound : void 8) {
        this.bound = that;
      } else {
    ...
    for (i = 0, _len = params.length; i < _len; ++i) {
      prm = params[i];
      if (prm.op === '=' && !prm.logic) {
        params[i] = Op('?', prm.left, prm.right);
      }
    }
    for (_i = 0, _len = params.length; _i < _len; ++_i) {
      prm = params[_i];
      if (prm instanceof Splat) {
        splatted = Assign(Arr((function(){
          var _i, _ref, _len, _results = [];
    ...
    for (_i = 0, _len = params.length; _i < _len; ++_i) {
      ...
      if (arg.isComplex()) {
        ...
        if (dfv) {
          val = Op(prm.op, ref, prm.second);
        }
        asns.push(Assign(arg, val));
        arg = ref;
      } else if (dfv) {
        asns.push(prm.op === '?'
          ? Op('&&', Literal(arg.value + ' == null'), Assign(arg, prm.second))
          : Assign(arg, prm.second, '=', prm.op));
      }
    ...
    if (splatted) {
      ...
      for (i = 0, _len = args.length; i < _len; ++i) {
        ...
        if (dic[0 + (a = a.compile(o))]) {
          throw SE('duplicate formal argument: ' + a);
        }
    ...
    if (this.statement) {
      if (!name) {
        throw SE('cannot declare a nameless function');
      }
      if (o.lines.scope !== pscope) {
        throw SE('cannot declare a function under a statement');
      }
  ...
  function paramName(o, node){
    ...
    if (prm.at) {
      ...
      if (name.reserved || (name === 'arguments' || name === 'eval')) {
        name = '$' + name;
      }
      prm = Literal(name);
  ...
  _proto.compileNode = function(o){
    ...
    if (this.title) {
      ...
      if (decl != null ? decl.reserved : void 8) {
        throw SE("reserved word \"" + decl + "\" cannot be a class name");
      }
    }
    name = decl || this.name;
    ...
    for (i = 0, _len = lines.length; i < _len; ++i) {
      ...
      if (node instanceof Obj) {
        ...
        if (ctor) {
          throw SE('more than one constructor in a class');
        }
        if ((ctor = node).bound) {
          proto = lname;
        }
...
exports.Super = Super = (function(_super){
  ...
  _proto.compile = function(o){
    ...
    if (!method) {
      throw SE('super outside of a function');
    }
    ...
    throw SE('super in an anonymous function');
  };
  return Super;
}(Node));
exports.Parens = Parens = (function(_super){
  ...
  _proto.compileNode = function(o){
    var it;
    it = this.it;
    if (!this.keep && !this.newed && ((it instanceof Literal || it instanceof Value || it instanceof Call || it instanceof Fun || it instanceof Parens) || it instanceof Op && o.level < LEVEL_OP)) {
...
_ref.add = function(name, type){
  ...
  case 'arguments':
  case 'eval':
    throw SE("redefining \"" + name + "\" is deprecated");
  }
