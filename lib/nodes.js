(function(){
  ...
  exports.Return = Return = (function(){
    ...
    _ref.compile = function(o, level){
      var exp, _ref;
      exp = (_ref = this.expression) != null ? _ref.makeReturn() : void 8;
      if (exp && !(exp instanceof Return)) {
        return exp.compile(o, level);
      } else {
        exp = (exp != null ? exp.expression : void 8) || '';
  ...
  exports.Import = Import = (function(){
    ...
    _ref.compileNode = function(o){
      var acc, code, com, delim, items, key, lref, node, ref, space, sub, top, val, _i, _len, _ref;
  ...
  exports.Obj = Obj = (function(){
    ...
    _ref.compileNode = function(o){
      var c, code, i, idt, items, last, node, rest, _i, _len;
      items = this.items;
      if (!items.length) {
        if (this.front) {
          return '({})';
      ...
      for (_i = 0, _len = items.length; _i < _len; ++_i) {
        ...
        code += idt + (node["this"] ? node.properties[0].name.value + ': ' + node.compile(o, LEVEL_LIST) : node instanceof Assign ? node.compile(o) : (c = node.compile(o, LEVEL_LIST)) + ': ' + c);
        if (node !== last) {
          code += ',';
        }
        code += '\n';
  ...
  exports.Class = Class = (function(){
    ...
    _ref.compileNode = function(o){
      ...
      if (this.title) {
        decl = this.title instanceof Value ? (_ref = (_ref2 = this.title.properties)[_ref2.length - 1].name) != null ? _ref.value : void 8 : this.title.value;
        if (decl) {
          if (decl.reserved) {
            throw SyntaxError("reserved word \"" + decl + "\" cannot be a class name");
          }
          if (!IDENTIFIER.test(decl)) {
            decl = '';
          }
        }
      }
  ...
  exports.Assign = Assign = (function(){
    ...
    _ref.compileDestructuring = function(o){
      ...
      list = left instanceof Arr ? this.destructArr(o, items, rite) : this.destructObj(o, items, rite);
      if (rref) {
        o.scope.free(rref);
      }
      if (cache) {
    ...
    _ref.destructArr = function(o, nodes, rite){
      var endi, i, iinc, ivar, lr, node, rest, val, _len, _results = [];
      iinc = '';
      for (i = 0, _len = nodes.length; i < _len; ++i) {
        ...
        if (node instanceof Splat) {
          ...
          if (i === (endi = nodes.length - 1)) {
            val = utility('slice') + (".call(" + rite) + (i ? ", " + i + ")" : ')');
          } else {
            val = ("" + nodes.length + " <= " + rite + ".length") + (" ? " + utility('slice') + ".call(" + rite + ", " + i);
            val += (rest = endi - i) ? (this.temps = [ivar = o.scope.temporary('i')], ", " + ivar + " = " + rite + ".length - " + rest + ") : (" + ivar + " = " + i + ", [])") : ') : []';
            iinc = ivar + '++';
          }
          val = Literal(val);
        } else {
  ...
  exports.Code = Code = (function(){
    ...
    _ref.compileNode = function(o){
      ...
      if (this.bound) {
        return utility('bind') + ("(" + this.bound + ", " + code + ")");
      }
      if (this.front) {
        return "(" + code + ")";
      } else {
    ...
    _ref.pluckDirectCalls = function(o, exps, name, index){
      ...
      for (idx = 0, _len = exps.length; idx < _len; ++idx) {
        ...
        if ((exp = exp.unwrapAll()) instanceof Call) {
          ...
          if (!(val instanceof Code && !exp.args.length || val instanceof Value && val.base instanceof Code && val.properties.length === 1 && ((_ref = val.properties[0].name) != null ? _ref.value : void 8) === 'call')) {
            continue;
          }
  ...
  exports.If = If = (function(){
    ...
    _ref.compileExpression = function(o){
      var code, _ref;
      code = this["if"].compile(o, LEVEL_COND) + ' ? ' + this.then.compile(o, LEVEL_LIST) + ' : ' + (((_ref = this["else"]) != null ? _ref.compile(o, LEVEL_LIST) : void 8) || 'void 8');
      if (o.level < LEVEL_COND) {
        return code;
      } else {
        return "(" + code + ")";
