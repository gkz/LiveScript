var Node, Negatable, Block, Literal, Key, Index, Value, Call, Clone, List, Obj, Arr, Op, Assign, Import, Of, Existence, Fun, Class, Super, Parens, Splat, Statement, Throw, Return, While, For, Try, Switch, Case, If, JS, Comment, Util, SE, UTILITIES, LEVEL_TOP, LEVEL_PAREN, LEVEL_LIST, LEVEL_COND, LEVEL_OP, LEVEL_ACCESS, TAB, IDENTIFIER, _ref, __extends = function(sub, sup){
...
Node = (function(){
  ...
  _proto.compileClosure = function(o){
    var that, fun, args, val, mentionsArgs;
    if (that = this.jumps()) {
  ...
  _proto.anaphorize = function(){
    ...
    if (this.eachChild(hasThat)) {
      ...
      base[name] = Assign(Literal('that'), base[name]);
    }
...
exports.Literal = Literal = (function(_super){
  ...
  _proto.isAssignable = function(){
    return !this.reserved && IDENTIFIER.test(this.value);
  };
...
exports.Value = Value = (function(_super){
  ...
  _proto.unfoldBind = function(o){
    var i, index, args, call, _ref, _len, _ref2;
    for (i = 0, _len = (_ref = this.tails).length; i < _len; ++i) {
      index = _ref[i];
      if ((_ref2 = index.symbol) === '~.' || _ref2 === '&.') {
        index.symbol = '';
...
exports.Obj = Obj = (function(_super){
  ...
  _proto.compileNode = function(o){
    var items, i, node, rest, code, idt, last, dic, key, fun, _len, _i;
    items = this.items;
    if (!items.length) {
      return (this.front ? '({})' : '{}');
    }
...
exports.Op = Op = (function(_super){
  ...
  function Op(op, first, second, post){
    var ps, i, p, args, call, _this = new _ctor;
    if (op === 'of') {
      return new Of(first, second);
    }
    if (op === 'do') {
      if (first instanceof Fun) {
        args = (ps = first.params).length && (function(){
          var _ref, _len, _results = [];
          for (i = 0, _len = (_ref = ps).length; i < _len; ++i) {
            p = _ref[i];
            if (p.op === '=' && !p.logic) {
              ps[i] = p.left;
              _results.push(p.right);
            } else {
              _results.push(p);
            }
          }
...
exports.Assign = Assign = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    if (!left.isAssignable()) {
      o.indent = '';
      o.level = LEVEL_LIST;
      throw SE(left.L('invalid assignment'));
    }
...
exports.Import = Import = (function(_super){
  ...
  function Import(left, right, all){
    ...
    _this.right = right;
    _this.all = all && 'All';
    if (!all && left instanceof Obj && right instanceof Obj) {
      return Obj(left.items.concat(right.items));
    }
    return _this;
  ...
  _proto.compileNode = function(o){
    var items, top, lref, sub, delim, space, code, node, com, dyna, key, val, fun, asn, _ref, _i, _len;
    if (this.all || !(this.right instanceof Obj)) {
...
exports.Of = Of = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    if (arr) {
      ...
      code = "" + utility('indexOf') + ".call(" + this.array.compile(o, lvl) + ", " + ref + ") " + (this.negated ? '<' : '>=') + " 0";
      if (sub === ref) {
        return code;
      }
      code = sub + ', ' + code;
...
exports.Fun = Fun = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    for (_i = 0, _len = params.length; _i < _len; ++_i) {
      ...
      if (arg.isEmpty()) {
        splatted || (arg = Literal(scope.temporary('arg')));
  ...
  function paramName(o, node){
    ...
    if (prm.at) {
      ...
      prm = Literal(dollarize(name));
    } else if (prm.isComplex() && !prm.isEmpty()) {
...
exports.Class = Class = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    if (this.title) {
      decl = dollarize(this.title instanceof Value
...
exports.Parens = Parens = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    if (!(this.keep || this.newed)) {
      if ((it instanceof Literal || it instanceof Value || it instanceof Call || it instanceof Fun || it instanceof Parens) || it instanceof Op && o.level < LEVEL_OP) {
...
exports.Switch = Switch = (function(_super){
  ...
  _proto.jumps = function(x){
    var cs, that, _i, _ref, _len;
    x == null && (x = {
      block: true
    });
    for (_i = 0, _len = (_ref = this.cases).length; _i < _len; ++_i) {
      cs = _ref[_i];
      if (that = cs.body.jumps(x)) {
        return that;
      }
    }
...
exports.JS = JS = (function(_super){
  ...
  _proto.compile = function(it){
    return tabify(this.code, it.indent);
  };
  return JS;
}(Node));
...
function tabify(code, tab){
  return code.replace(/\n/g, '\n' + tab);
}
function dollarize(it){
  if (it && (it.reserved || (it === 'arguments' || it === 'eval'))) {
    return '$' + it;
  } else {
    return it;
  }
