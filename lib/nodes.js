(function(){
  ...
  exports.For = For = (function(){
    ...
    __importAll(For.prototype, {
      ...
      compileNode: function(o){
        var body, code, cond, defPart, eq, forPart, idt, index, ivar, lvar, name, namePart, ownPart, pattern, pvar, retPart, scope, sourcePart, step, svar, tail, tvar, vars, _ref;
        scope = o.scope;
        body = this.body;
        code = ownPart = defPart = retPart = '';
        idt = o.indent + TAB;
        pattern = this.name && !(this.name instanceof Literal);
        ...
        if (this.from) {
          ...
          namePart = pattern ? Assign(this.name, Literal("" + svar + "[" + ivar + "]")).compile(o, LEVEL_TOP) : name ? "" + name + " = " + svar + "[" + ivar + "]" : void 0;
          if (!this.object) {
            if (sourcePart !== svar) {
        ...
        if (this.object) {
          ...
          if (this.own) {
            ownPart = "if (" + utility('owns') + ".call(" + svar + ", " + ivar + ")) ";
          }
        } else {
          if (step !== pvar) {
            vars += ', ' + step;
        ...
        return defPart + this.tab + ("for (" + forPart + ") " + ownPart + "{" + code + "}") + retPart;
      },
