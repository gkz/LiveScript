var Node, Negatable, Lines, Literal, Key, Index, Value, Call, Clone, Obj, Arr, Op, Assign, Import, Of, Existence, Fun, Class, Super, Parens, Splat, Statement, Throw, Return, While, For, Try, Switch, Case, If, Comment, Util, Scope, UTILITIES, LEVEL_TOP, LEVEL_PAREN, LEVEL_LIST, LEVEL_COND, LEVEL_OP, LEVEL_ACCESS, TAB, IDENTIFIER, METHOD_DEF, utility, lastNonComment, _ref, __extends = function(sub, sup){
...
exports.Switch = Switch = (function(_super){
  ...
  _proto.compileNode = function(o){
    var cond, tab, code, stop, i, cs, def, _ref, _len;
...
exports.Case = Case = (function(_super){
  ...
  _proto.compileCase = function(o, tab, nobr, bool){
    var tests, i, test, br, code, exps, last, ft, that, _to, _i, _len, _ref;
    tests = this.tests;
    i = -1;
    while (test = tests[++i]) {
      if (test instanceof Arr) {
        tests.splice.apply(tests, [i, 1].concat(test.items));
        i += test.items.length - 1;
      }
    }
    if (bool) {
      test = tests[0];
      for (i = 1, _to = tests.length; i < _to; ++i) {
        test = Op('||', test, tests[i]);
      }
      (this.test = test, this.aSource = 'test', this.aTarget = 'body', this).anaphorize();
      tests = [this.test.invert()];
    }
    code = br = '';
    for (_i = 0, _len = tests.length; _i < _len; ++_i) {
      test = tests[_i];
      code += tab + ("case " + test.compile(o, LEVEL_PAREN) + ":\n");
    }
