(function(){
  var Scope, Node, Lines, Literal, Statement, Throw, Return, Value, Comment, Call, Extends, Import, Access, Index, Obj, Arr, Class, Assign, Fun, Param, Splat, While, Op, Of, Try, Existence, Parens, For, Switch, Case, If, Super, UTILITIES, LEVEL_TOP, LEVEL_PAREN, LEVEL_LIST, LEVEL_COND, LEVEL_OP, LEVEL_ACCESS, TAB, IDENTIFIER, SIMPLENUM, METHOD_DEF, utility, multident, lastNonComment, __extends = function(sub, sup){
  ...
  exports.Lines = Lines = (function(){
    ...
    _ref.compileWithDeclarations = function(o){
      var post, code, i, node, rest, vars, _ref, _len, _ref2;
      o.level = LEVEL_TOP;
      code = post = '';
      for (i = 0, _len = (_ref = this.lines).length; i < _len; ++i) {
        node = _ref[i];
        if (!((_ref2 = node.unwrap()) instanceof Comment || _ref2 instanceof Literal)) {
          break;
        }
      }
      if (i) {
  ...
  exports.Value = Value = (function(){
    ...
    _ref.compileNode = function(o){
      var asn, val, tails, code, t, _i, _len;
      ...
      tails = (val = this.substituteStar(o) || this).tails;
      code = val.head.compile(o, LEVEL_ACCESS);
      if (tails[0] instanceof Access && SIMPLENUM.test(code)) {
        code += ' ';
      }
      for (_i = 0, _len = tails.length; _i < _len; ++_i) {
        t = tails[_i];
        code += t.compile(o);
      }
      return code;
    ...
    _ref.substituteStar = function(o){
      var find, i, tail, star, sub, ref, _ref, _len, _ref2;
      find = function(it){
        switch (false) {
        case it.value !== '*':
          return it;
      ...
      for (i = 0, _len = (_ref = this.tails).length; i < _len; ++i) {
        tail = _ref[i];
        if (!(tail instanceof Index && (star = tail.traverseChildren(find)))) {
          continue;
        }
  ...
  exports.Import = Import = (function(){
    ...
    _ref.compileNode = function(o){
      var rite, items, top, lref, sub, delim, space, code, node, com, dyna, key, val, acc, asn, _ref, _i, _len;
      rite = this.right.unwrap();
      if (!(this.util === 'import' && rite instanceof Obj)) {
        return Call(Value(Literal(utility(this.util))), [this.left, rite]).compile(o);
      }
      items = rite.items;
      if (!items.length) {
  ...
  exports.Class = Class = (function(){
    ...
    _ref.compileNode = function(o){
      ...
      for (i = 0, _len = lines.length; i < _len; ++i) {
        node = lines[i];
        if ((node = node.unwrap()) instanceof Obj) {
  ...
  exports.Assign = Assign = (function(){
    ...
    _ref.compileNode = function(o){
      ...
      if ((left = this.left.unwrap()).items) {
        if (!this.logic) {
    ...
    _ref.compileConditional = function(o, left){
      var ref, _ref;
      _ref = Value(left).cacheReference(o), left = _ref[0], ref = _ref[1];
      return Op(this.logic, left, Assign(ref, this.right, this.op)).compile(o);
    };
    _ref.compileDestructuring = function(o, left){
  ...
  exports.Fun = Fun = (function(){
    ...
    _ref.compileNode = function(o){
      var pscope, sscope, scope, params, body, name, tab, code, b, vars, asns, param, p, splats, ref, val, exps, wasEmpty, i, v, _ref, _i, _len;
      pscope = o.scope;
  ...
  exports.Op = Op = (function(){
    ...
    _ref.compileNode = function(o){
      ...
      if (this.op === 'instanceof' && this.second.unwrap() instanceof Arr) {
  ...
  exports.Of = Of = (function(){
    ...
    function Of(item, arr){
      var _this = new _ctor;
      _this.item = item;
      _this.array = arr.unwrap();
      return _this;
    } Of.name = 'Of';
    _ref = Of.prototype;
  ...
  exports.For = For = (function(){
    ...
    _ref.pluckDirectCalls = function(o){
      ...
      return this.body.eachChild(dig = function(it){
        ...
        if (!(it instanceof Call && (fn = it.callee.unwrapAll()) instanceof Fun && fn.params.length === it.args.length)) {
  ...
  exports.Switch = Switch = (function(){
    ...
    function Switch($switch, cases, $default){
      var tests, i, _i, _len, _this = new _ctor;
      _this['switch'] = $switch;
      _this.cases = cases;
    ...
    _ref.compileNode = function(o){
      var tab, code, stop, i, cs, def, _ref, _len;
      tab = this.tab;
