...
exports.Call = Call = (function(_super){
  ...
  _proto.unfoldSoak = function(o){
    var that, lr, call, ifn, _ref, _i, _len;
    if (this.soak) {
      if (that = If.unfoldSoak(o, this, 'callee')) {
        return that;
      }
      lr = this.callee.cacheReference(o);
      return If(JS("typeof " + lr[0].compile(o) + " == 'function'"), Value((_ref = Call(lr[1], this.args), _ref['new'] = this['new'], _ref)), {
        soak: true
      });
    }
  ...
  _proto.compileNode = function(o){
    ...
    callee = this.callee;
    callee.isCallable() || callee.carp(callee.compile(o, LEVEL_ACCESS) + ' // invalid callee');
    if (that = this.method && this['new'] || Splat.compileArray(o, this.args, true)) {
      if (this['new']) {
  ...
  _proto.compileSplat = function(o, args){
    var base, name, ref, fun;
    (base = Value(this.callee)).front = this.front;
  ...
  Call.back = function(params, arrow, call){
    var args, i, a, _len;
    call instanceof Call || (call = Call(call));
    args = call.args;
...
exports.Op = Op = (function(_super){
  ...
  function Op(op, first, second, post){
    var args, ps, method, i, p, call, _len, _ref, _this = new _ctor;
    switch (op) {
    case 'of':
  ...
  _proto.compileUnary = function(o){
    ...
    if (op === 'new' && !first.isCallable()) {
      first.carp(code + ' // invalid constructor');
    }
    if (this.post) {
      code += op;
    } else {
