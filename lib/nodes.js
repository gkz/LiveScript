var Node, Negatable, Block, Atom, Literal, Var, Key, Index, Chain, Call, Clone, List, Obj, Arr, Op, Assign, Import, Of, Existence, Fun, Class, Super, Parens, Splat, Statement, Throw, Return, While, For, Try, Switch, Case, If, JS, Util, UTILITIES, LEVEL_TOP, LEVEL_PAREN, LEVEL_LIST, LEVEL_COND, LEVEL_OP, LEVEL_ACCESS, PREC, TAB, IDENTIFIER, _ref, __import = function(obj, src){
...
Node = (function(){
  ...
  _proto.compileClosure = function(o){
    ...
    return Parens(Chain(fun, [(_ref = Call(args), _ref.method = method, _ref)]), true).compileNode(o);
  };
...
exports.Chain = Chain = (function(_super){
  var SIMPLENUM, _proto = __extends(Chain, _super).prototype;
  function _ctor(){} _ctor.prototype = _proto;
  ...
  _proto.multiPick = function(o){
    ...
    for (i = 0, _len = (_ref = this.tails).length; i < _len; ++i) {
      ...
      if (key != null ? key.length : void 8) {
        _ref2 = Chain(this.head, this.tails.slice(0, i)).cache(o), sub = _ref2[0], ref = _ref2[1], this.temps = _ref2[2];
        arr = (function(){
          var _ref, _len, _results = [];
          for (j = 0, _len = (_ref = key).length; j < _len; ++j) {
            node = _ref[j];
            _results.push(Chain(j ? ref : sub, [Index(node)]));
          }
          return _results;
        }());
        return Chain(Arr(arr), this.tails.slice(i + 1));
      }
    }
  };
  return Chain;
...
exports.Op = Op = (function(_super){
  ...
  function Op(op, first, second, post){
    ...
    case 'do':
      ...
      return Parens(Chain(first).add((_ref = Call(args), _ref.method = method, _ref)));
    case 'new':
      first.newed = true;
...
exports.Assign = Assign = (function(_super){
  ...
  _proto.rendObj = function(o, nodes, rite){
    ...
    for (_i = 0, _len = nodes.length; _i < _len; ++_i) {
      ...
      val = Chain(lr || (lr = Var(rite)), [Index(key.asKey())]);
      if (splat) {
        val = Import(Obj(), val);
      }
      _results.push(Assign(node, val, this.op).compile(o, LEVEL_TOP));
  ...
  _proto.transleft = function(o){
    ...
    if (left instanceof Clone) {
      ...
      left = Obj(items);
    } else if (left instanceof Chain && (tail = (_ref = left.tails)[_ref.length - 1]) instanceof Index && tail.key.length) {
      ...
      left = Arr((function(){
        ...
        for (i = 0, _len = (_ref = key).length; i < _len; ++i) {
          k = _ref[i];
          _results.push(Chain(i ? ref : sub, [Index(k)]));
        }
        return _results;
      }()));
    }
...
exports.Import = Import = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    for (_i = 0, _len = items.length; _i < _len; ++_i) {
      ...
      asn = Assign(Chain(lref, [Index(dyna ? key : key.asKey())]), val);
...
exports.Parens = Parens = (function(_super){
  ...
  _proto.compileNode = function(o){
    ...
    if (!(this.keep || this.newed)) {
      if ((it instanceof Atom || it instanceof Chain || it instanceof Fun || it instanceof Parens) || it instanceof Op && o.level < LEVEL_OP + (PREC[it.op] || PREC.unary)) {
