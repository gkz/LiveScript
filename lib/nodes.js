...
exports.Op = Op = (function(_super){
  ...
  _proto.compileIOS = function(o, items){
    var code, sub, ref, i, item, _ref, _len;
    code = '';
    _ref = this.first.cache(o, false, LEVEL_OP), sub = _ref[0], ref = _ref[1];
    for (i = 0, _len = items.length; i < _len; ++i) {
      item = items[i];
      code += "" + (i ? ' || ' + ref : sub) + " instanceof " + item.compile(o, LEVEL_OP);
    }
    if (sub !== ref) {
      o.scope.free(ref);
    }
...
exports.Of = Of = (function(_super){
  ...
  _proto.compileNode = function(o){
    var items, lvl, sub, ref, code, cmp, cnj, i, item, _ref, _len;
    _ref = this.item.cache(o, false, lvl = (items = this.array.selections()) ? LEVEL_OP : LEVEL_LIST), sub = _ref[0], ref = _ref[1];
    if (items) {
      code = '';
      _ref = this.negated
        ? [' !== ', ' && ']
      ...
      for (i = 0, _len = items.length; i < _len; ++i) {
        item = items[i];
        code += (i ? cnj + ref : sub) + cmp + item.compile(o, lvl);
      }
    } else {
