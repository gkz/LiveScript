(function(){
  var Scope, Node, Lines, Literal, Statement, Throw, Return, Value, Comment, Call, Extends, Import, Access, Index, Obj, Arr, Class, Assign, Fun, While, Op, Of, Try, Existence, Parens, Splat, For, Switch, Case, If, Super, UTILITIES, LEVEL_TOP, LEVEL_PAREN, LEVEL_LIST, LEVEL_COND, LEVEL_OP, LEVEL_ACCESS, TAB, IDENTIFIER, METHOD_DEF, utility, multident, lastNonComment, __extends = function(sub, sup){
    function ctor(){} ctor.prototype = (sub.superclass = sup).prototype;
    return (sub.prototype = new ctor).constructor = sub;
  }, __owns = Object.prototype.hasOwnProperty, __importAll = function(obj, src){ for (var key in src) obj[key] = src[key]; return obj };
  ...
  exports.Lines = Lines = (function(_super){
    ...
    _proto.compileWithDeclarations = function(o){
      var post, code, i, node, rest, vars, _ref, _len;
      o.level = LEVEL_TOP;
      code = post = '';
  ...
  exports.Statement = Statement = (function(_super){
    ...
    _proto.jumps = function(it){
      return !(it && (it.loop || it.block && this.name !== 'continue'));
    };
    _proto.compile = function(it){
      return it.indent + this.name + ';';
    };
    return Statement;
  }(Node));
  exports.Throw = Throw = (function(_super){
    ...
    _proto.compile = function(o){
      ...
      return o.indent + this.name + (exp ? ' ' + exp : '') + ';';
    };
    return Throw;
  }(Statement));
  ...
  exports.Value = Value = (function(_super){
    var SIMPLENUM, STARSEEK, _proto = __extends(Value, _super).prototype;
    function _ctor(){} _ctor.prototype = _proto;
    ...
    _proto.compileNode = function(o){
      var val, base, rest, t, _i, _ref, _len;
      if (val = this.unfoldAssign(o) || this.unfoldBind(o)) {
      ...
      rest = '';
      for (_i = 0, _len = (_ref = val.tails).length; _i < _len; ++_i) {
        t = _ref[_i];
        rest += t.compile(o);
      }
    ...
    _proto.poleStar = function(o){
      var i, t, star, sub, ref, _ref, _len, _ref2;
      for (i = 0, _len = (_ref = this.tails).length; i < _len; ++i) {
        t = _ref[i];
        if (!(t instanceof Index && (star = t.traverseChildren(STARSEEK)))) {
          continue;
        }
    ...
    _proto.unfoldSoak = function(o){
      var ifn, i, tail, fst, snd, ref, _ref, _len;
    ...
    _proto.unfoldAssign = function(o){
      var asn, i, tail, lhs, rhs, _ref, _len, _ref2;
    ...
    _proto.unfoldBind = function(o){
      var i, ps, p, args, _ref, _len;
      for (i = 0, _len = (_ref = ps = this.tails).length; i < _len; ++i) {
        p = _ref[i];
        if (p.bind) {
          p.bind = false;
          args = Value(this.head, ps.slice(0, i)).cache(o, true);
  ...
  exports.Call = Call = (function(_super){
    ...
    _proto.unfoldSoak = function(o){
      var left, rite, call, ifn, _ref, _i, _len;
      if (this.soak) {
        if (_ref = If.unfoldSoak(o, this, 'callee')) {
          return _ref;
        }
    ...
    _proto.compileNode = function(o){
      var asn, fun, args, arg, code;
      if (asn = this.unfoldAssign(o)) {
        return asn.compile(o);
      }
      if (!(fun = (this.callee.head || this.callee) instanceof Fun)) {
  ...
  exports.Extends = Extends = (function(_super){
    var _proto = __extends(Extends, _super).prototype;
    function _ctor(){} _ctor.prototype = _proto;
  ...
  exports.Import = Import = (function(_super){
    ...
    _proto.compileNode = function(o){
      var items, top, lref, sub, delim, space, code, node, com, dyna, key, val, acc, asn, _ref, _i, _len;
      ...
      for (_i = 0, _len = items.length; _i < _len; ++_i) {
        ...
        acc = !dyna && key instanceof Literal && IDENTIFIER.test(key.value);
  ...
  exports.Access = Access = (function(_super){
    var _proto = __extends(Access, _super).prototype;
    function _ctor(){} _ctor.prototype = _proto;
    function Access(key, symbol){
      var _this = new _ctor;
      _this.key = key;
      _this.symbol = symbol;
      switch (symbol) {
    ...
    _proto.compile = function(o){
      var key;
      if ((key = this.key.compile(o)).charAt(0) === "'") {
        return "[" + key + "]";
      } else {
        return "." + key;
      }
  ...
  exports.Index = Index = (function(_super){
    ...
    _proto.children = ['index'];
    _proto.show = Access.prototype.show;
    _proto.compile = function(o){
  ...
  exports.Obj = Obj = (function(_super){
    ...
    _proto.compileNode = function(o){
      var items, i, node, rest, last, idt, dic, code, key, _len, _i;
      items = this.items;
      if (!items.length) {
        return (this.front ? '({})' : '{}');
      }
      ...
      for (_i = 0, _len = items.length; _i < _len; ++_i) {
        ...
        code += idt + (node.at
          ? (key = node.tails[0].key.compile(o), key + ': ' + node.compile(o, LEVEL_LIST))
  ...
  exports.Arr = Arr = (function(_super){
    ...
    _proto.children = ['items'];
    _proto.assigns = Obj.prototype.assigns;
    _proto.compileNode = function(o){
      var obj, code, _ref;
      if (!this.items.length) {
        return '[]';
      }
      if (_ref = Splat.compileArray(o, this.items)) {
        return _ref;
      }
      o.indent += TAB;
      code = (function(){
    ...
    _proto.compileNode = function(o){
      ...
      if (this.title) {
        decl = this.title instanceof Value
          ? (_ref = (_ref2 = this.title.tails)[_ref2.length - 1].key) != null ? _ref.value : void 8
          : this.title.value;
      ...
      if (this.sup) {
        args.push(this.sup);
        proto = Extends(lname, (_ref = fun.params)[_ref.length] = Literal('_super'));
      }
  ...
  exports.Assign = Assign = (function(_super){
    var _proto = __extends(Assign, _super).prototype;
    function _ctor(){} _ctor.prototype = _proto;
    ...
    _proto.compileNode = function(o){
      var left, name, right, match, val, code;
      left = this.left;
      if (left.items) {
        if (!this.logic) {
      ...
      right = this.right;
      if ((right instanceof Fun || right instanceof Class) && (match = METHOD_DEF.exec(name))) {
        if (match[1]) {
          right.clas = match[1];
        }
    ...
    _proto.compileConditional = function(o){
      var lef, ref, _ref;
      _ref = Value(this.left).cacheReference(o), lef = _ref[0], ref = _ref[1];
      return Op(this.logic, lef, Assign(ref, this.right, this.op)).compile(o);
    };
    _proto.compileDestructuring = function(o){
      ...
      if ((len > 1 || o.level) && (!IDENTIFIER.test(rite) || this.left.assigns(rite))) {
        cache = "" + (rref = o.scope.temporary('ref')) + " = " + rite;
        rite = rref;
      }
      list = this.left instanceof Arr
    ...
    _proto.destructObj = function(o, nodes, rite){
      var node, splat, dyna, key, acc, lr, val, _i, _len, _ref, _results = [];
  ...
  exports.Fun = Fun = (function(_super){
    var paramName, _proto = __extends(Fun, _super).prototype;
    function _ctor(){} _ctor.prototype = _proto;
    ...
    _proto.compileNode = function(o){
      var pscope, sscope, scope, params, body, name, tab, code, b, args, asns, prm, p, splats, arg, asn, ref, val, a, exps, wasEmpty, i, _ref, _i, _len;
      pscope = o.scope;
      ...
      if (!splats) {
        for (i = 0, _len = args.length; i < _len; ++i) {
          a = args[i];
          scope.parameter(args[i] = a.compile(o));
        }
      }
    ...
    paramName = function(o, node){
      ...
      if (prm.at) {
        prm = prm.tails[0].key;
        if (prm.value.reserved) {
          prm = Literal('$' + prm.value);
        }
  ...
  exports.While = While = (function(_super){
    ...
    _proto.children = ['condition', 'body'];
    _proto.isStatement = YES;
    ...
    _proto.addBody = function(body){
      ...
      if (last instanceof Statement && last.name === 'continue') {
  ...
  exports.Op = Op = (function(_super){
    ...
    function Op(op, first, second, post){
      ...
      if (op === 'new') {
        ...
        if ((p = first.head || first) instanceof Parens) {
          p.keep = true;
        }
      }
      _this.op = op;
    ...
    _proto.show = Assign.prototype.show;
    _proto.invert = function(){
      var op, _ref;
  ...
  exports.Existence = Existence = (function(_super){
    ...
    _proto.children = ['it'];
    _proto.show = Of.prototype.show;
    _proto.invert = Of.prototype.invert;
    _proto.compileNode = function(o){
      var code;
  ...
  exports.Parens = Parens = (function(_super){
    ...
    _proto.children = ['it'];
    _proto.isComplex = function(it){
      return this.it.isComplex();
    };
    _proto.compileNode = function(o){
      var it;
      it = this.it;
      if (!this.keep && ((it instanceof Value || it instanceof Call || it instanceof Fun || it instanceof Parens) || o.level < LEVEL_OP && it instanceof Op)) {
  ...
  exports.For = For = (function(_super){
    ...
    _proto.children = ['source', 'name', 'from', 'to', 'step', 'body'];
    _proto.compileNode = function(o){
      var idx, step, pvar, eq, tail, tvar, vars, cond, srcPart, svar, lvar, forPart, ownPart, head, item, body, _ref;
      if (this.index instanceof Node) {
        if (!(this.index instanceof Literal)) {
    ...
    _proto.pluckDirectCalls = function(o){
      ...
      return this.body.eachChild(dig = function(it){
        ...
        if (!(it instanceof Call && (fn = it.callee.unwrap()) instanceof Fun && fn.params.length === it.args.length)) {
  ...
  exports.Switch = Switch = (function(_super){
    ...
    _proto.compileNode = function(o){
      var cond, tests, i, tab, code, stop, cs, def, _ref, _i, _len;
      if (!(cond = ((_ref = this['switch']) != null ? _ref.compile(o, LEVEL_PAREN) : void 8) || false)) {
        for (_i = 0, _len = (_ref = this.cases).length; _i < _len; ++_i) {
          tests = _ref[_i].tests;
  ...
  exports.Case = Case = (function(_super){
    ...
    _proto.compileCase = function(o, tab, nobr){
      var br, code, add, test, t, exps, last, i, ft, body, _i, _ref, _len, _j, _ref2, _len2;
      code = br = '';
      add = function(it){
      ...
      if (!(nobr || ft || last instanceof Statement && last.name !== 'debugger')) {
        code += o.indent + 'break;\n';
      }
      return code;
    };
  ...
  exports.If = If = (function(_super){
    ...
    function If($if, then, _arg){
      ...
      _this['if'] = $if;
      _this.then = then;
      _ref = _arg != null ? _arg : {}, _this.soak = _ref.soak, _this.post = _ref.post, name = _ref.name;
    ...
    _proto.compileStatement = function(o){
      var ref, code, cond, body, _ref;
      if (this.post && this.then instanceof Return && !this.then.it) {
        _ref = this['if'].cache(o, true), this['if'] = _ref[0], ref = _ref[1];
        this.then = Return(ref);
      }
