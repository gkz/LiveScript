...
exports.Binary = Binary = (function(superclass){
  ...
  prototype.xorChildren = function(test){
    var second, first, result;
    result = (first = test(this.first) || (second = test(this.second))) && !(first && second);
    if (!result) {
      return false;
    }
    return first
  ...
  prototype.compileNode = function(o){
    var top, rite, items, that, level, code;
    if (this.partial) {
    ...
    default:
      if (COMPARER.test(this.op)) {
        if (that = this.xorChildren(function(it){
          return it.isRegex();
        })) {
          return this.compileRegexEquals(o, that);
        }
        if (that = this.xorChildren((function(it){
          return it instanceof Arr || it instanceof Obj;
        }))) {
          return this.compileObjEquals(o, that.concat(Literal("'" + this.op + "'")));
        }
  ...
  prototype.compileObjEquals = function(o, args){
    return Chain(Var(util('eq'))).add(Call(args)).compile(o);
  };
...
exports.Switch = Switch = (function(superclass){
  ...
  function Switch(type, topic, cases, $default){
    ...
    if (type === 'match') {
      if (topic) {
        this.target = topic.length > 1
          ? (this.apply = true, Arr(topic))
          : topic[0];
      }
      this.topic = null;
    } else {
  ...
  prototype.compileNode = function(o){
    var tab, targetNode, target, topic, code, stop, i, c, that, __ref, __len;
    tab = this.tab;
    if (this.target) {
    ...
    for (i = 0, __len = (__ref = this.cases).length; i < __len; ++i) {
      c = __ref[i];
      code += c.compileCase(o, tab, i === stop, !topic, this.type, this.apply, target, i === 0 ? targetNode : void 8);
    }
    if (this['default']) {
      o.indent = tab + TAB;
...
exports.Case = Case = (function(superclass){
  ...
  prototype.compileCase = function(o, tab, nobr, bool, type, apply, target, targetNode){
    ...
    if (type === 'match') {
      for (i = 0, __len = tests.length; i < __len; ++i) {
        ...
        tests[i] = apply
          ? Chain(test).add(Index(Key('apply', '.', true))).add(Call([Literal('this'), target]))
          : Chain(test).add(Call(target
            ? [target]
            : []));
      }
    }
