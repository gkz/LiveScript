...
exports.For = For = (function(superclass){
  ...
  prototype.infuseIIFE = function(o){
    var _this = this;
    function dup(fun, name){
      var p, _i, _ref, _len;
      if (name) {
        for (_i = 0, _len = (_ref = fun.params).length; _i < _len; ++_i) {
          p = _ref[_i];
          if (name === p.value) {
            return true;
          }
    ...
    this.body.traverseChildren(function(it){
      ...
      if (_this.index && !dup(fun, _this.index)) {
        fun.params.push((_ref = call.args)[_ref.length] = Var(_this.index));
      }
      if (name = _this.name) {
        if (name.isComplex()) {
          name = Var(_this.nref || (_this.nref = (_ref = _this.temps)[_ref.length] = o.scope.temporary('ref')));
        } else if (dup(fun, name.value)) {
          return;
        }
        fun.params.push(_this.name);
        call.args.push(name);
      }
    });
  };
