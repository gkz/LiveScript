...
exports.Op = Op = (function(_super){
  ...
  prototype.compileUnary = function(o){
    var op, it, them, i, node, splat, code, _len, _ref;
    op = this.op;
    if ((it = this.first.expandSlice(o).unwrap()) instanceof Arr && (them = it.items).length) {
      for (i = 0, _len = them.length; i < _len; ++i) {
        node = them[i];
        if (node instanceof Splat) {
          node.it = Op(op, node.it, null, this.post);
          splat = true;
        } else {
          them[i] = Op(op, node, null, this.post);
        }
      }
      if (!splat && (this['void'] || !o.level)) {
...
exports.Assign = Assign = (function(_super){
  ...
  prototype.compileNode = function(o){
    var left, op, right, reft, lvar, sign, name, res, code, del, that, _ref;
    left = this.left.unwrap().expandSlice(o, true).unwrap();
    if (left.isEmpty()) {
