...
exports.For = For = (function(superclass){
  ...
  prototype.compileNode = function(o){
    ...
    if (this.name && !this.name.isEmpty()) {
      head += '\n' + o.indent + Assign(this.name, JS(svar + "[" + idx + "]")).compile(o, LEVEL_TOP) + ';';
    }
  ...
  prototype.infuseIIFE = function(){
    ...
    this.body.traverseChildren(function(it){
      var fun, params, call, index, name;
      ...
      index = _this.index, name = _this.name;
      if (index && !dup(params, index)) {
        call.args.push(params[params.length] = Var(index));
      }
      if (name && !name.isComplex() && !dup(params, name.value)) {
        call.args.push(params[params.length] = name);
      }
    });
  };
  return For;
