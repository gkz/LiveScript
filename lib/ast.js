...
exports.Assign = Assign = (function(superclass){
  ...
  prototype.compileNode = function(o){
    var left, ref$, i$, len$, op, right, reft, lvar, sign, name, protoSplit, dotSplit, empty, res, code, del, that;
...
exports.Class = Class = (function(superclass){
  ...
  prototype.compile = function(o, level){
    var fun, body, lines, title, boundFuncs, decl, name, proto, ctorName, ctor, importProtoObj, i, len$, node, ref$, i$, f, vname, args, that, imports, res$, clas;
    ...
    importProtoObj = function(node){
      var j, ref$, len$, prop, ref1$, i$, len1$, f;
      for (j = 0, len$ = (ref$ = node.items).length; j < len$; ++j) {
        prop = ref$[j];
        if (!prop) {
          continue;
        }
        if ((ref1$ = prop.key) instanceof Key || ref1$ instanceof Literal) {
          if ((prop.key instanceof Key && prop.key.name === ctorName) || (prop.key instanceof Literal && prop.key.value === "'" + ctorName + "'")) {
            ...
            for (i$ = 0, len1$ = (ref1$ = prop.val).length; i$ < len1$; ++i$) {
              f = ref1$[i$];
              f.meth = prop.key;
            }
          }
    ...
    if (!(ctor instanceof Fun)) {
      lines.unshift(Assign(Var(ctorName), ctor));
      lines.unshift(ctor = Fun([], Block(Chain(Var(ctorName)).add(Call([Splat('arguments', true)])))));
    }
    ctor.name = name;
    ctor.ctor = true;
    ctor.statement = true;
      ...
      if (it instanceof Block) {
        for (i = 0, len$ = (ref$ = it.lines).length; i < len$; ++i) {
          ...
          if (child instanceof Obj) {
            it.lines[i] = importProtoObj(child);
          }
        }
      }
    }
...
exports.Require = Require = (function(superclass){
  ...
  prototype.children = ['list'];
  prototype.compile = function(o){
    var stripString, getFileName, res$, i$, ref$, len$, item, chain, ref1$, asg, value, main, out;
    ...
    getFileName = function(val){
      var ref$;
      return (ref$ = stripString(val).split('/'))[ref$.length - 1].split('.')[0];
    };
    res$ = [];
    for (i$ = 0, len$ = (ref$ = this.list.items).length; i$ < len$; ++i$) {
      item = ref$[i$];
      ref1$ = (fn$.call(this)), asg = ref1$[0], value = ref1$[1];
      main = Chain(Var('require')).add(Call([Literal("'" + value + "'")]));
      res$.push(Assign(Var(asg), chain ? Chain(main, chain) : main).compile(o));
    }
    out = res$;
    return out.join(";\n" + o.indent);
    function fn$(){
      ...
      case !(item instanceof Prop):
        return [
          item.key instanceof Key
            ? item.key.name
            : getFileName(item.key.value), item.val instanceof Var
            ? item.val.value
            : item.val instanceof Chain
              ? (chain = item.val.tails, stripString(item.val.head instanceof Key
                ? item.val.head.name
                : item.val.head.value))
              : stripString(item.val.value)
        ];
      case !(item instanceof Literal):
        return [getFileName(item.value), stripString(item.value)];
      default:
        return this.carp('unsupported require type');
      }
    }
  };
  return Require;
