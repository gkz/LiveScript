...
exports.Class = Class = (function(superclass){
  ...
  prototype.compile = function(o, level){
    var fun, body, lines, title, boundFuncs, decl, name, proto, ctorName, i, len$, node, j, ref$, len1$, prop, ref1$, ctor, i$, len2$, f, vname, args, that, imports, res$, clas;
    ...
    for (i = 0, len$ = lines.length; i < len$; ++i) {
      ...
      if (node instanceof Obj) {
        ...
        for (j = 0, len1$ = (ref$ = node.items).length; j < len1$; ++j) {
          prop = ref$[j];
          if (!prop) {
            continue;
          }
          if ((ref1$ = prop.key) instanceof Key || ref1$ instanceof Literal) {
            if ((prop.key instanceof Key && prop.key.name === ctorName) || (prop.key instanceof Literal && prop.key.value === "'" + ctorName + "'")) {
              ...
              for (i$ = 0, len2$ = (ref1$ = prop.val).length; i$ < len2$; ++i$) {
                f = ref1$[i$];
                f.meth = prop.key;
              }
            }
