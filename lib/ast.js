...
exports.Of = Of = (function(_super){
  ...
  prototype.compileNode = function(o){
    var level, sub, ref, arr, tests, code, cmp, cnj, i, test, _ref, _len;
    _ref = this.item.cache(o, false, level = LEVEL_LIST), sub = _ref[0], ref = _ref[1];
    if ((arr = this.array.expandSlice(o).unwrap()) instanceof Arr && (tests = arr.items).length > 1) {
      code = '';
      level = LEVEL_OP + PREC['=='];
      _ref = this.negated
        ? [' !== ', ' && ']
        : [' === ', ' || '], cmp = _ref[0], cnj = _ref[1];
      for (i = 0, _len = tests.length; i < _len; ++i) {
        test = tests[i];
        code && (code += cnj);
        if (test instanceof Splat) {
          code += (_ref = new Of(Var(ref), test.it), _ref.negated = this.negated, _ref).compile(o, level);
          if (!(i || sub === ref)) {
            code = "(" + sub + ", " + code + ")";
          }
        } else {
