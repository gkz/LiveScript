...
exports.Let = function(type, args, body, lno){
  var bodyless, d, that, x$;
  if (body == null) {
    bodyless = true;
    body = Block();
  }
  if (type === 'letvar' || type === 'letconst') {
    d = Decl(type.substr(3), args, lno);
    body.prepend.apply(body, (that = d.lines) != null
      ? that
      : [d]);
    args = [];
  }
  x$ = Call['let'](args, body);
  if (bodyless) {
    x$.back = body;
  }
  return x$;
};
exports.Decl = Decl = function(type, nodes, lno){
  if (!nodes[0]) {
