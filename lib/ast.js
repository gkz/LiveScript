...
exports.Chain = Chain = (function(_super){
  ...
  prototype.expandArray = function(o){
    var tails, i, that, sub, ref, temps, j, key, items, _ref, __results, _len;
    tails = this.tails;
    i = -1;
    while (that = tails[++i]) {
...
exports.Assign = Assign = (function(_super){
  ...
  prototype.compileNode = function(o){
    var right, left, head, tails, decl, sign, name, res, code, del, that, _ref;
    right = this.right;
  ...
  prototype.compileConditional = function(o, left){
    ...
    return Op(this.logic, lefts[0], (this.left = lefts[1], this.logic = false, this)).compileNode(o);
  };
...
exports.Splat = Splat = (function(_super){
  ...
  Splat.compileArray = function(o, list, apply){
    var index, node, args, atoms, base, _len, _i, _ref;
    ...
    if (list.length === 1) {
      node = (apply ? Object : ensureArray)(list[0].it);
      return node.compile(o, LEVEL_LIST);
    }
    args = [];
    atoms = [];
    for (_i = 0, _len = (_ref = list.splice(index, 9e9)).length; _i < _len; ++_i) {
      ...
      if (node instanceof Splat) {
        ...
        args.push(ensureArray(node.it).compile(o, LEVEL_LIST));
      } else {
        atoms.push(node);
      }
    }
  ...
  function ensureArray(node){
    var _ref;
    while (node instanceof Parens) {
      node = node.it;
    }
    node = node.unwrap();
    if ((node instanceof Arr || node instanceof While) || node instanceof Chain && ((_ref = (_ref = node.tails)[_ref.length - 1].key) != null ? _ref.length : void 8) || node.op === '*' && node.first instanceof Arr) {
      return node;
    }
