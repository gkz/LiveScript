...
exports.Assign = Assign = (function(_super){
  ...
  prototype.compileNode = function(o){
    var right, left, head, tails, op, name, res, code, del, that, _ref;
    right = this.right;
    ...
    if (right instanceof Parens && !right.keep) {
      right = right.it;
    }
    ...
    code = !o.level && right instanceof While
      ? (right.makeReturn(res = o.scope.temporary('_results')), res + " = [];\n" + this.tab + "" + right.compile(o) + "\n" + this.tab + "" + name + " " + op + " " + o.scope.free(res))
      : (name + " " + op + " ") + (right.assigned = true, right).compile(o, LEVEL_LIST);
  ...
  prototype.rendArr = function(o, nodes, rite){
    var i, node, len, empty, val, ivar, start, inc, lr, _len, _ref, _results = [];
    for (i = 0, _len = nodes.length; i < _len; ++i) {
      ...
      _results.push((_ref = Assign(node, val, this.op), _ref['void'] = true, _ref).compile(o, LEVEL_PAREN));
    }
    return _results;
  };
