...
exports.Chain = Chain = (function(superclass){
  ...
  prototype.add = function(it){
    ...
    } else if (it instanceof Call && this.head instanceof Parens && this.tails.length === 1 && this.head.it instanceof Binary && ((__ref = this.head.it.op) == '&&' || __ref == '||' || __ref == 'xor')) {
      return Chain(Parens(Binary(this.head.it.op, Chain(this.head.it.first).add(it), Chain(this.head.it.second).add(it))));
    }
    return this;
  };
