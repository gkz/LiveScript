...
exports.Op = Op = (function(_super){
  ...
  prototype.isArray = function(){
    ...
    case '/':
      return this.second.isString() || this.second instanceof JS;
    }
  };
  EQUALITY = /^[!=]==?$/;
  COMPARER = /^(?:[!=]=|[<>])=?$/;
  ...
  prototype.compileJoin = function(o){
    if (this.first.isArray()) {
      return Chain(this.first, [Index(Key('join')), Call([this.second])]).compile(o);
    } else {
      return Call.make(JS(utility('join') + '.call'), [this.first, this.second]).compile(o);
    }
  };
  prototype.compileSplit = function(o){
    if (this.first.isString()) {
      return Chain(this.first, [Index(Key('split')), Call([this.second])]).compile(o);
    } else {
      return Call.make(JS(utility('split') + '.call'), [this.first, this.second]).compile(o);
    }
  };
...
exports.Assign = Assign = (function(_super){
  ...
  prototype.compileNode = function(o){
    var op, right, left, lefts, head, tails, decl, sign, name, res, code, del, that, _ref;
    ...
    if (op === '**=' || op === '+=' && right instanceof Arr || op === '*=' && right.isString() || op === '/=' && (right.isString() || right instanceof JS)) {
      left = (lefts = Chain(left).cacheReference(o))[0];
      right = Op(op.slice(0, -1), lefts[1], right);
      op = ':=';
    }
  ...
  prototype.rendArr = function(o, nodes, rite){
    ...
    for (i = 0, _len = nodes.length; i < _len; ++i) {
      ...
      if (node instanceof Splat) {
        if (ivar) {
          node.carp('multiple splat in an assignment');
        }
        len = nodes.length;
...
exports.Fun = Fun = (function(_super){
  ...
  prototype.compileNode = function(o){
    var pscope, sscope, scope, params, body, name, tab, code, that, lines, wasEmpty, args, asns, i, p, splat, dic, arg, dfv, ref, a, _ref, _len, _i;
    pscope = o.scope;
...
exports.If = If = (function(_super){
  ...
  prototype.isArray = function(){
    var _ref;
    return ((_ref = this['else']) != null ? _ref.isArray() : void 8) && this.then.isArray();
  };
  prototype.jumps = function(it){
    var _ref;
