...
exports.Obj = Obj = (function(superclass){
  ...
  Obj.prototype.toSlice = function(o, base, symbol, assign){
    var items, ref$, ref, temps, i$, len$, i, node, inPlace, val, name, logic, keylessParen, key, chain;
    items = this.items;
    if (items.length > 1) {
    ...
    for (i$ = 0, len$ = items.length; i$ < len$; ++i$) {
      ...
      if (inPlace = node instanceof Prop || node instanceof Splat) {
        val = node[name = (ref$ = node.children)[ref$.length - 1]];
      } else {
        val = node;
      }
      if (logic = val.getDefault()) {
        val = val.first;
      }
      (val instanceof Literal || val instanceof Var || val instanceof Parens || val instanceof List) || this.carp("value in object slice is not a key");
      if (keylessParen = val instanceof Parens && !(node instanceof Prop)) {
        ref$ = val.cache(o, true), key = ref$[0], val = ref$[1];
        if (assign) {
      ...
      val = chain = Chain(base, [Index(val.maybeKey(), symbol)]);
      if (logic) {
  ...
  Obj.prototype.compileNode = function(o){
    ...
    for (i$ = 0, len$ = items.length; i$ < len$; ++i$) {
      ...
      } else if (node instanceof Key || node instanceof Var || node instanceof Literal) {
        code.push(key = node.compile(o), ": ", key);
      } else {
        this.carp("invalid property shorthand");
      }
