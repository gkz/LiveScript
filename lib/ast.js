...
exports.Chain = Chain = (function(superclass){
  ...
  prototype.add = function(it){
    ...
    } else if (it instanceof Call && this.head instanceof Parens && this.tails.length === 1 && this.head.it instanceof Binary && __in(this.head.it.op, logics = ['&&', '||', 'xor'])) {
      call = it;
      f = function(x, key){
        var y;
        y = x[key];
        if (y instanceof Binary && __in(y.op, logics)) {
          ...
          return x[key] = Chain(y).add(call);
        }
      };
      f(this.head.it, 'first');
      f(this.head.it, 'second');
      return this.head.it;
    }
    return this;
  };
