var Node, Negatable, Block, Atom, Literal, Var, Key, Index, Chain, Call, Clone, List, Obj, Prop, Arr, Op, Assign, Import, Of, Existence, Fun, Class, Super, Parens, Splat, Jump, Throw, Return, While, For, Try, Switch, Case, If, Label, JS, Util, UTILITIES, LEVEL_TOP, LEVEL_PAREN, LEVEL_LIST, LEVEL_COND, LEVEL_OP, LEVEL_CALL, PREC, TAB, IDENTIFIER, SIMPLENUM, _ref, __import = function(obj, src){
...
(Node = function(){
  ...
  compileLoopReference: function(o, name, level){
    var tmp, _ref;
    if (this instanceof Var && o.scope.check(this.value) || this instanceof Literal || this.op === '-' && (-1 / 0 < (_ref = +this.first.value) && _ref < 1 / 0)) {
      return [tmp = this.compile(o), tmp];
    }
    return [tmp = o.scope.temporary(name), Assign(Var(tmp), this).compile(o, level)];
  },
    ...
    for (i = 0, _len = (_ref = this.tails).length; i < _len; ++i) {
      ...
      if (node.soak) {
        ...
        return node instanceof Call
          ? ((_ref2 = Chain(this.head, this.tails.splice(0, i)).cacheReference(o), test = _ref2[0], this.head = _ref2[1], _ref2), If(JS("typeof " + test.compile(o, LEVEL_OP) + " == 'function'"), this, {
            soak: true
          }))
          : ((_ref2 = Chain(this.head, this.tails.splice(0, i)).unwrap().cache(o, true), test = _ref2[0], this.head = _ref2[1], _ref2), If(Existence(test), this, {
            soak: true
          }));
      }
    }
    ...
    for (i = 0, _len = items.length; i < _len; ++i) {
      ...
      code += idt + (node instanceof Prop
        ? ((key = node.key, val = node.val, node), node.accessor
          ...
          : (key instanceof Key && (val instanceof Fun || val instanceof Class) ? val.name = key.varName() : void 8, (key = key.compile(o)) + ": " + val.compile(o, LEVEL_LIST)))
...
exports.Op = Op = (function(_super){
  ...
  prototype.compileDelete = function(o){
    ...
    code = this.assigned ? '' : (ref = o.scope.temporary('ref')) + " = ";
  ...
  prototype.compileRepeat = function(o){
    var x, n, ref, items, refs, i, item, q, _ref, _len;
    x = this.first, n = this.second;
    if (!(n instanceof Literal && (1 < (_ref = n.value) && _ref < 0x100))) {
      ref = "repeat" + (x instanceof Arr ? 'Array' : 'String');
      return Call.make(Util(ref), [x, n]).compile(o);
    }
    n = +n.value;
    if (x instanceof Arr) {
      items = x.items;
      ...
      return '[' + List.compile(o, items) + __repeatString(", " + List.compile(o, refs), n - 1) + ']';
    } else if (x instanceof Literal) {
      return (q = (x = x.compile(o)).charAt()) + __repeatString("" + x.slice(1, -1), n) + q;
    } else {
      _ref = x.cache(o, 1, LEVEL_OP), x = _ref[0], ref = _ref[1];
      x += __repeatString(" + " + ref, n - 1);
...
exports.Assign = Assign = (function(_super){
  ...
  prototype.compileNode = function(o){
    var right, left, name, that, val, del, code, _ref;
    right = this.right;
    ...
    val = (right.assigned = true, right).compile(o, LEVEL_LIST);
...
exports.Super = Super = (function(_super){
  ...
  prototype.compile = function(o){
    ...
    if (name = fun.name) {
      if (that = fun.clas) {
        return that + '.superclass.prototype' + (IDENTIFIER.test(name)
          ? '.' + name
          : '[' + name + ']');
...
exports.While = While = (function(_super){
  ...
  prototype.pluckDirectCalls = function(o){
    ...
    return this.body.eachChild(dig = function(it){
      ...
      _ref = it = it.it, fun = _ref.head, call = _ref.tails[0];
      if (fun.statement) {
        return;
      }
...
exports.For = For = (function(_super){
  ...
  prototype.compileNode = function(o){
    var temps, idx, pvar, step, eq, tvar, tail, vars, cond, svar, srcPart, lvar, head, item, that, body, _ref;
    temps = this.temps = [];
    if (idx = this.index) {
      o.scope.declare(idx);
    } else {
    ...
    if (!this.object) {
      _ref = (this.step || Literal(1)).compileLoopReference(o, 'step', LEVEL_TOP), pvar = _ref[0], step = _ref[1];
      pvar === step || temps.push(pvar);
    }
    if (this.from) {
      eq = this.op === 'til' ? '' : '=';
      _ref = this.to.compileLoopReference(o, 'to', LEVEL_TOP), tvar = _ref[0], tail = _ref[1];
      ...
      if (this.name || this.object && this.own) {
        _ref = this.source.compileLoopReference(o, 'ref', this.object ? LEVEL_TOP : LEVEL_CALL), svar = _ref[0], srcPart = _ref[1];
...
exports.Label = Label = (function(_super){
  ...
  _ref = Parens.prototype, prototype.children = _ref.children, prototype.isCallable = _ref.isCallable;
  prototype.show = function(){
    return this.label;
  };
  prototype.isStatement = YES;
