...
Coco.compile = function(code, options){
  var fname, _ref;
  options || (options = {});
  fname = (_ref = options.filename, delete options.filename, _ref);
  try {
    return parser.parse(lex(code)).compileRoot(options);
  } catch (e) {
    if (fname) {
      e.message = "in " + fname + ", " + e.message;
    }
    throw e;
  }
};
...
if ((fs = require('fs')) && (path = require('path'))) {
  Coco.run = function(code, options){
    ...
    root.filename = (that = options != null ? options.filename : void 8) ? fs.realpathSync(that) : '.';
