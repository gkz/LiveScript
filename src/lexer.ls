...
exports import
  ...
  doJS: (code, JSTOKEN.lastIndex) ->
    ...
    @token \LITERAL Object(detab js.slice(1 -1), @dent) <<< {+js}
  ...
  doLine: (code, index) ->
    ...
    if 0 > delta = tabs.length - @dent
      ...
      or tag in <[ +- => DOT LOGIC MATH COMPARE RELATION SHIFT BITWISE
  ...
  doLiteral: (code, index) ->
    ...
    @unline! if tag in <[ , => DOT LOGIC COMPARE MATH IMPORT SHIFT BITWISE ]>
    @token tag, val
    sym.length
...
!function addImplicitIndentation tokens
  ...
  while token = tokens[++i]
    ...
    switch
    # ->,
    case next in <[ DOT \? \, \=> ]> then --i; fallthrough
    # -> 0,
...
!function addImplicitParentheses tokens
  ...
  function ok token, i
    ...
    case \WHILE
      ...
      fallthrough
    case <[ NEWLINE POST_IF FOR BY TO ]>
      return pre.0 is not \,
    false
...
!function addImplicitBraces tokens
  ...
  while token = tokens[++i]
    ...
    pre   = tokens[index-1]
    continue unless pre.0 in <[ : ASSIGN ]> or stack[*-1]?0 is not \{
    stack.push [\{]
